...
public class StarmapScreen : KModalScreen
{
  ...
  protected override void OnShow(bool show)
  {
    ...
    {
      AudioMixer.instance.Stop((HashedString) AudioMixerSnapshots.Get().MENUStarmapSnapshot);
  ...
  private void ScrollToBottom()
  {
    ...
    Vector3 position = new Vector3((float) x, (float) num, (float) z);
  ...
  private void FillRocketListPanel()
  {
    ...
    foreach (Spacecraft spacecraft2 in spacecraft1)
    {
      ...
      Tuple<string, BreakdownListRow.Status> textForState = StarmapScreen.GetTextForState(rocket.state);
      ...
      if ((UnityEngine.Object) component8 != (UnityEngine.Object) null)
      {
        ...
        int num = count == 0 ? 1 : 2;
        breakdownListRow2.ShowStatusData(passengers, str, (BreakdownListRow.Status) num);
      }
      if (rocket.state == Spacecraft.MissionState.Grounded)
      {
        ...
        if (this.selectedDestination != null)
        {
          if (SpacecraftManager.instance.GetDestinationAnalysisState(this.selectedDestination) == SpacecraftManager.DestinationAnalysisState.Complete)
          {
            ...
            breakdownListRow4.ShowStatusData((string) STRINGS.UI.STARMAP.LISTTITLES.DESTINATION_MASS, this.DisplayDestinationMass(this.selectedDestination), flag ? BreakdownListRow.Status.Default : BreakdownListRow.Status.Yellow);
        ...
        {
          breakdownListRow4.ShowStatusData((string) STRINGS.UI.STARMAP.DESTINATIONSELECTION.NOTSELECTED, "", BreakdownListRow.Status.Red);
      ...
      else
      {
        ...
        component1.AddRow().ShowData((string) STRINGS.UI.STARMAP.ROCKETSTATUS.TIMEREMAINING, Util.FormatOneDecimalPlace(timeLeft / 600f) + " / " + GameUtil.GetFormattedCycles(duration));
    ...
    Spacecraft.MissionState state)
  {
    switch (state)
    {
      ...
      case Spacecraft.MissionState.Underway:
        return new Tuple<string, BreakdownListRow.Status>((string) STRINGS.UI.STARMAP.MISSION_STATUS.UNDERWAY, BreakdownListRow.Status.Red);
    ...
    foreach (GameObject gameObject in AttachableBuilding.GetAttachedNetwork(this.currentCommandModule.GetComponent<AttachableBuilding>()))
    {
      ...
      if ((UnityEngine.Object) component1 != (UnityEngine.Object) null)
      {
        ...
        if (engineFuelTag.IsValid)
        {
          ...
          breakdownListRow.ShowData(gameObject.gameObject.GetProperName() + " (" + element.name + ")", GameUtil.GetFormattedMass(component1.GetAmountAvailable(engineFuelTag), massFormat: GameUtil.MetricMassFormat.Tonne));
        }
        else
        {
    ...
    if ((UnityEngine.Object) lcmToVisualize != (UnityEngine.Object) null && spacecraft != null)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          Vector2 vector2_2 = num2 == 0 ? new Vector2(Mathf.Lerp(a.x, b.x, num1 * 2f), Mathf.Lerp(a.y, b.y, num1 * 2f)) : new Vector2(Mathf.Lerp(a.x, b.x, (float) (1.0 - (double) num1 * 2.0 + 1.0)), Mathf.Lerp(a.y, b.y, (float) (1.0 - (double) num1 * 2.0 + 1.0)));
