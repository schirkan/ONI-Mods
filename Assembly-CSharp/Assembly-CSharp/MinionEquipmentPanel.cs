...
public class MinionEquipmentPanel : KMonoBehaviour
{
  ...
  public MinionEquipmentPanel() => this.refreshDelegate = new System.Action<object>(this.Refresh);

  protected override void OnPrefabInit()
  {
    base.OnPrefabInit();
    this.roomPanel = Util.KInstantiateUI(ScreenPrefabs.Instance.CollapsableContentPanel, this.gameObject);
    ...
    this.ownablePanel = Util.KInstantiateUI(ScreenPrefabs.Instance.CollapsableContentPanel, this.gameObject);
    ...
    foreach (AssignableSlotInstance slot in assignables.Slots)
    {
      if (slot.slot.showInUI)
      {
        ...
        if (slot.IsAssigned())
        {
          ...
          flag = true;
          string str = slot.IsAssigned() ? slot.assignable.GetComponent<KSelectable>().GetName() : UI.DETAILTABS.PERSONALITY.EQUIPMENT.NO_ASSIGNABLES.text;
  ...
  private string GetAssignedEffectsString(AssignableSlotInstance slot)
  {
    ...
    descriptorList.AddRange((IEnumerable<Descriptor>) GameUtil.GetGameObjectEffects(slot.assignable.gameObject));
