...
public class MinionIdentity : KMonoBehaviour, ISaveLoadable, IAssignableIdentity, IListableOption, ISim1000ms
{
  ...
  private static readonly EventSystem.IntraObjectHandler<MinionIdentity> OnDeadTagChangedDelegate = GameUtil.CreateHasTagHandler<MinionIdentity>(GameTags.Dead, (System.Action<MinionIdentity, object>) ((component, data) => component.OnDied(data)));
  ...
  private void ApplyCustomGameSettings()
  {
    ...
    if (currentQualitySetting2.id == "Doomed")
      Db.Get().Amounts.Stress.deltaAttribute.Lookup((Component) this).Add(new AttributeModifier(Db.Get().Amounts.Stress.deltaAttribute.Id, 0.03333334f, (string) UI.FRONTEND.CUSTOMGAMESETTINGSSCREEN.SETTINGS.STRESS.LEVELS.DOOMED.ATTRIBUTE_MODIFIER_NAME));
    else if (currentQualitySetting2.id == "Pessimistic")
      Db.Get().Amounts.Stress.deltaAttribute.Lookup((Component) this).Add(new AttributeModifier(Db.Get().Amounts.Stress.deltaAttribute.Id, 0.01666667f, (string) UI.FRONTEND.CUSTOMGAMESETTINGSSCREEN.SETTINGS.STRESS.LEVELS.PESSIMISTIC.ATTRIBUTE_MODIFIER_NAME));
    else if (currentQualitySetting2.id == "Optimistic")
      Db.Get().Amounts.Stress.deltaAttribute.Lookup((Component) this).Add(new AttributeModifier(Db.Get().Amounts.Stress.deltaAttribute.Id, -0.01666667f, (string) UI.FRONTEND.CUSTOMGAMESETTINGSSCREEN.SETTINGS.STRESS.LEVELS.OPTIMISTIC.ATTRIBUTE_MODIFIER_NAME));
    ...
    if (currentQualitySetting3.id == "VeryHard")
      Db.Get().Amounts.Calories.deltaAttribute.Lookup((Component) this).Add(new AttributeModifier(Db.Get().Amounts.Calories.deltaAttribute.Id, -1666.667f, (string) UI.FRONTEND.CUSTOMGAMESETTINGSSCREEN.SETTINGS.CALORIE_BURN.LEVELS.VERYHARD.ATTRIBUTE_MODIFIER_NAME));
