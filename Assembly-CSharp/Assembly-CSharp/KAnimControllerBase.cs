...
public abstract class KAnimControllerBase : MonoBehaviour
{
  ...
  public KAnimHashedString GetBuildHash() => this.curBuild == null ? (KAnimHashedString) KAnimBatchManager.NO_BATCH : this.curBuild.fileHash;
  ...
  public void Play(HashedString[] anim_names, KAnim.PlayMode mode = KAnim.PlayMode.Once)
  {
    ...
    Debug.Assert((uint) anim_names.Length > 0U, (object) "Play was called with an empty anim array");
  ...
  public bool HasAnimation(HashedString anim_name) => this.anims.ContainsKey(anim_name) || this.overrideAnims.ContainsKey(anim_name);
  ...
  public KAnimFile[] AnimFiles
  {
    ...
    set
    {
      DebugUtil.Assert((uint) value.Length > 0U, "Controller for " + this.gameObject.name + " has no anim files.");
      DebugUtil.Assert((UnityEngine.Object) value[0] != (UnityEngine.Object) null, "First anim file for " + this.gameObject.name + " needs to be non-null.");
      DebugUtil.Assert(value[0].buildBytes != null, "First anim file for " + this.gameObject.name + " needs to be the build file.");
