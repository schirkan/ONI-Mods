...
namespace Klei.AI
{
  ...
  public abstract class Disease : Resource
  {
      ...
      ElemGrowthInfo default_value)
    {
      ...
      int elementIndex1 = ElementLoader.GetElementIndex(SimHashes.Polypropylene);
      ElemGrowthInfo elemGrowthInfo1 = new ElemGrowthInfo();
      elemGrowthInfo1.underPopulationDeathRate = 2.666667f;
      ...
      int elementIndex2 = ElementLoader.GetElementIndex(SimHashes.Vacuum);
      ...
      if (this.growthRules != null)
      {
        for (int index = 0; index < this.growthRules.Count; ++index)
        {
          if (this.growthRules[index] is TagGrowthRule growthRule && tags.Contains(growthRule.tag))
            num *= Disease.HalfLifeToGrowthRate((overpopulated ? growthRule.overPopulationHalfLife : growthRule.populationHalfLife).Value, 1f);
        }
      }
      return num;
    }
