  ...
  public class Effect : Modifier
  {
    ...
    public static string CreateTooltip(Effect effect, bool showDuration, string linePrefix = "\n")
    {
      ...
      if (showDuration && (double) effect.duration > 0.0)
        str = str + linePrefix + string.Format((string) DUPLICANTS.MODIFIERS.TIME_TOTAL, (object) GameUtil.GetFormattedCycles(effect.duration));
      return str;
    }
      ...
      bool increase_indent = false)
    {
      ...
      {
        Descriptor descriptor = new Descriptor((string) Strings.Get("STRINGS.DUPLICANTS.ATTRIBUTES." + selfModifier.AttributeId.ToUpper() + ".NAME") + ": " + selfModifier.GetFormattedString(parent), "");
        if (increase_indent)
