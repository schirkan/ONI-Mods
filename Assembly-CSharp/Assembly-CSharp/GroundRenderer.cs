...
public class GroundRenderer : KMonoBehaviour
{
  ...
  private struct WorldChunk
  {
          ...
          for (int index1 = 0; index1 < GroundRenderer.WorldChunk.uniqueElements.Length; ++index1)
          {
            ...
            if (uniqueElement.IsSolid)
            {
              ...
              if (idx != num5)
              {
                ...
                if (index2 > 0)
                {
                  GroundMasks.UVData[] variationUvs = biomeMask.tiles[index2].variationUVs;
    ...
    private static int GetBiomeIdx(int cell) => !Grid.IsValidCell(cell) ? 0 : (int) World.Instance.zoneRenderData.GetSubWorldZoneType(cell);
