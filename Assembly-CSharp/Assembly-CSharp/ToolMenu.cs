...
public class ToolMenu : KScreen
{
    ...
    for (int index1 = 0; index1 < collections.Count; ++index1)
    {
      ...
      if (tc.tools != null)
      {
        ...
        for (int index2 = 0; index2 < tc.tools.Count; ++index2)
        {
          ...
          tc.UIMenuDisplay.GetComponent<ExpandRevealUIContent>().Collapse(closure_0 ?? (closure_0 = (System.Action<object>) (s =>
          {
  ...
  private void ChooseCollection(ToolMenu.ToolCollection collection, bool autoSelectTool = true)
  {
    ...
    this.rows.ForEach((System.Action<List<ToolMenu.ToolCollection>>) (row => this.OpenOrCloseCollectionsInRow((IList<ToolMenu.ToolCollection>) row)));
  }
