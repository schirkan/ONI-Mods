...
public class DetectorNetwork : GameStateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>
{
  ...
  public class NetworkStates : GameStateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>.State
  {
    ...
    public DetectorNetwork.NetworkStates InitializeStates(DetectorNetwork parent)
    {
      ...
      this.poor.ToggleStatusItem((string) BUILDING.STATUSITEMS.NETWORKQUALITY.NAME, (string) BUILDING.STATUSITEMS.NETWORKQUALITY.TOOLTIP, icon_type: StatusItem.IconType.Exclamation, notification_type: NotificationType.BadMinor, resolve_string_callback: new Func<string, DetectorNetwork.Instance, string>(this.StringCallback)).ParamTransition<float>((StateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>.Parameter<float>) parent.networkQuality, this.good, (StateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>.Parameter<float>.Callback) ((smi, p) => (double) p >= 0.8));
      this.good.ToggleStatusItem((string) BUILDING.STATUSITEMS.NETWORKQUALITY.NAME, (string) BUILDING.STATUSITEMS.NETWORKQUALITY.TOOLTIP, resolve_string_callback: new Func<string, DetectorNetwork.Instance, string>(this.StringCallback)).ParamTransition<float>((StateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>.Parameter<float>) parent.networkQuality, this.good, (StateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>.Parameter<float>.Callback) ((smi, p) => (double) p < 0.8));
      return this;
    }
  ...
  public new class Instance : GameStateMachine<DetectorNetwork, DetectorNetwork.Instance, IStateMachineTarget, DetectorNetwork.Def>.GameInstance
  {
    ...
    public static int ScanVisiblityLine(int start_cell, int x_offset, int y_offset, int radius)
    {
      ...
      for (int index = 0; Mathf.Abs(index) <= radius; ++index)
      {
        ...
        {
          if (Grid.ExposedToSunlight[num2] >= (byte) 253)
            ++num1;
          else
            break;
        }
