...
public abstract class BaseWireConfig : IBuildingConfig
{
    ...
    EffectorValues noise)
  {
    ...
    BuildingDef buildingDef = BuildingTemplates.CreateBuildingDef(id1, 1, 1, anim1, 10, (float) num, construction_mass1, allMetals, 1600f, BuildLocationRule.Anywhere, decor1, noise1);
  ...
  protected void DoPostConfigureComplete(Wire.WattageRating rating, GameObject go)
  {
    ...
    descriptor.SetupDescriptor(string.Format((string) UI.BUILDINGEFFECTS.MAX_WATTAGE, (object) GameUtil.GetFormattedWattage(maxWattageAsFloat)), string.Format((string) UI.BUILDINGEFFECTS.TOOLTIPS.MAX_WATTAGE, (object[]) Array.Empty<object>()));
