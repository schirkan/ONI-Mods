...
public class Navigator : StateMachineComponent<Navigator.StatesInstance>, ISaveLoadableDetails, ISim4000ms
{
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.log = new LoggerFS(nameof (Navigator));
  ...
  public bool IsMoving() => this.smi.IsInsideState((StateMachine.BaseState) this.smi.sm.moving);
  ...
  public void UpdateTarget(int cell) => this.targetLocator.transform.SetPosition(Grid.CellToPosCBC(cell, Grid.SceneLayer.Move));
  ...
  private bool ValidatePath(ref PathFinder.Path path) => PathFinder.ValidatePath(this.NavGrid, this.GetCurrentAbilities(), ref path);
  ...
  public void AdvancePath(bool trigger_advance = true)
  {
    ...
    else
    {
      ...
      if (flag)
      {
        ...
        {
          PathFinder.PotentialPath potential_path = new PathFinder.PotentialPath(cell1, this.CurrentNavType, this.flags);
  ...
  public NavGrid.Transition GetNextTransition() => this.NavGrid.transitions[this.path.nodes[1].transitionId];
  ...
  public void Stop(bool arrived_at_destination = false, bool play_idle = true)
  {
    ...
    if (play_idle)
    {
      ...
      this.GetComponent<KAnimControllerBase>().Play(idleAnim, KAnim.PlayMode.Loop);
    }
  ...
  public class States : GameStateMachine<Navigator.States, Navigator.StatesInstance, Navigator>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.moving.Enter((StateMachine<Navigator.States, Navigator.StatesInstance, Navigator, object>.State.Callback) (smi => smi.Trigger(1027377649, (object) GameHashes.ObjectMovementWakeUp))).Update("UpdateNavigator", (System.Action<Navigator.StatesInstance, float>) ((smi, dt) => smi.master.SimEveryTick(dt)), UpdateRate.SIM_EVERY_TICK, true).Exit((StateMachine<Navigator.States, Navigator.StatesInstance, Navigator, object>.State.Callback) (smi => smi.Trigger(1027377649, (object) GameHashes.ObjectMovementSleep)));
      this.arrived.TriggerOnEnter(GameHashes.DestinationReached).GoTo(this.stopped);
      this.failed.TriggerOnEnter(GameHashes.NavigationFailed).GoTo(this.stopped);
