...
public static class DiscreteShadowCaster
{
    ...
    LightShape shape)
  {
    ...
    {
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.N_NW, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.N_NE, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.E_NE, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.E_SE, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.S_SE, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.S_SW, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.W_SW, 1.0, 0.0, visiblePoints);
      DiscreteShadowCaster.ScanOctant(xy, range, 1, DiscreteShadowCaster.Octant.W_NW, 1.0, 0.0, visiblePoints);
    }
    else
    {
    ...
    switch (octant)
    {
      case DiscreteShadowCaster.Octant.N_NW:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num4, (double) num3, (double) cellPos.x, (double) cellPos.y, false) <= endSlope; ++num4)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num4, num3))
            {
              if (num4 - 1 >= 0 && !DiscreteShadowCaster.DoesOcclude(num4 - 1, num3) && !DiscreteShadowCaster.DoesOcclude(num4 - 1, num3 - 1))
                DiscreteShadowCaster.ScanOctant(cellPos, range, depth + 1, octant, startSlope, DiscreteShadowCaster.GetSlope((double) num4 - 0.5, (double) num3 - 0.5, (double) cellPos.x, (double) cellPos.y, false), visiblePoints);
            }
            else
            {
              if (num4 - 1 >= 0 && DiscreteShadowCaster.DoesOcclude(num4 - 1, num3))
                startSlope = -DiscreteShadowCaster.GetSlope((double) num4 - 0.5, (double) num3 + 0.5, (double) cellPos.x, (double) cellPos.y, false);
              if (!DiscreteShadowCaster.DoesOcclude(num4, num3 - 1) && !visiblePoints.Contains(Grid.XYToCell(num4, num3)))
      ...
      case DiscreteShadowCaster.Octant.N_NE:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num5, (double) num3, (double) cellPos.x, (double) cellPos.y, false) >= endSlope; --num5)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num5, num3))
            {
              if (num5 + 1 < Grid.HeightInCells && !DiscreteShadowCaster.DoesOcclude(num5 + 1, num3) && !DiscreteShadowCaster.DoesOcclude(num5 + 1, num3 - 1))
              {
            ...
            {
              if (num5 + 1 < Grid.HeightInCells && DiscreteShadowCaster.DoesOcclude(num5 + 1, num3))
                startSlope = DiscreteShadowCaster.GetSlope((double) num5 + 0.5, (double) num3 + 0.5, (double) cellPos.x, (double) cellPos.y, false);
              if (!DiscreteShadowCaster.DoesOcclude(num5, num3 - 1) && !visiblePoints.Contains(Grid.XYToCell(num5, num3)))
      ...
      case DiscreteShadowCaster.Octant.E_NE:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num2, (double) num6, (double) cellPos.x, (double) cellPos.y, true) >= endSlope; --num6)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num2, num6))
            {
              if (num6 + 1 < Grid.HeightInCells && !DiscreteShadowCaster.DoesOcclude(num2, num6 + 1) && !DiscreteShadowCaster.DoesOcclude(num2 - 1, num6 + 1))
                DiscreteShadowCaster.ScanOctant(cellPos, range, depth + 1, octant, startSlope, DiscreteShadowCaster.GetSlope((double) num2 - 0.5, (double) num6 + 0.5, (double) cellPos.x, (double) cellPos.y, true), visiblePoints);
            }
            else
            {
              if (num6 + 1 < Grid.HeightInCells && DiscreteShadowCaster.DoesOcclude(num2, num6 + 1))
                startSlope = DiscreteShadowCaster.GetSlope((double) num2 + 0.5, (double) num6 + 0.5, (double) cellPos.x, (double) cellPos.y, true);
              if (!DiscreteShadowCaster.DoesOcclude(num2 - 1, num6) && !visiblePoints.Contains(Grid.XYToCell(num2, num6)))
      ...
      case DiscreteShadowCaster.Octant.E_SE:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num2, (double) num7, (double) cellPos.x, (double) cellPos.y, true) <= endSlope; ++num7)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num2, num7))
            {
              if (num7 - 1 >= 0 && !DiscreteShadowCaster.DoesOcclude(num2, num7 - 1) && !DiscreteShadowCaster.DoesOcclude(num2 - 1, num7 - 1))
                DiscreteShadowCaster.ScanOctant(cellPos, range, depth + 1, octant, startSlope, DiscreteShadowCaster.GetSlope((double) num2 - 0.5, (double) num7 - 0.5, (double) cellPos.x, (double) cellPos.y, true), visiblePoints);
            }
            else
            {
              if (num7 - 1 >= 0 && DiscreteShadowCaster.DoesOcclude(num2, num7 - 1))
                startSlope = -DiscreteShadowCaster.GetSlope((double) num2 + 0.5, (double) num7 - 0.5, (double) cellPos.x, (double) cellPos.y, true);
              if (!DiscreteShadowCaster.DoesOcclude(num2 - 1, num7) && !visiblePoints.Contains(Grid.XYToCell(num2, num7)))
      ...
      case DiscreteShadowCaster.Octant.S_SE:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num8, (double) num3, (double) cellPos.x, (double) cellPos.y, false) <= endSlope; --num8)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num8, num3))
            {
              if (num8 + 1 < Grid.WidthInCells && !DiscreteShadowCaster.DoesOcclude(num8 + 1, num3) && !DiscreteShadowCaster.DoesOcclude(num8 + 1, num3 + 1))
              {
            ...
            {
              if (num8 + 1 < Grid.WidthInCells && DiscreteShadowCaster.DoesOcclude(num8 + 1, num3))
                startSlope = -DiscreteShadowCaster.GetSlope((double) num8 + 0.5, (double) num3 - 0.5, (double) cellPos.x, (double) cellPos.y, false);
              if (!DiscreteShadowCaster.DoesOcclude(num8, num3 + 1) && !visiblePoints.Contains(Grid.XYToCell(num8, num3)))
      ...
      case DiscreteShadowCaster.Octant.S_SW:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num9, (double) num3, (double) cellPos.x, (double) cellPos.y, false) >= endSlope; ++num9)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num9, num3))
            {
              if (num9 - 1 >= 0 && !DiscreteShadowCaster.DoesOcclude(num9 - 1, num3) && !DiscreteShadowCaster.DoesOcclude(num9 - 1, num3 + 1))
              {
            ...
            {
              if (num9 - 1 >= 0 && DiscreteShadowCaster.DoesOcclude(num9 - 1, num3))
                startSlope = DiscreteShadowCaster.GetSlope((double) num9 - 0.5, (double) num3 - 0.5, (double) cellPos.x, (double) cellPos.y, false);
              if (!DiscreteShadowCaster.DoesOcclude(num9, num3 + 1) && !visiblePoints.Contains(Grid.XYToCell(num9, num3)))
      ...
      case DiscreteShadowCaster.Octant.W_SW:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num2, (double) num10, (double) cellPos.x, (double) cellPos.y, true) >= endSlope; ++num10)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num2, num10))
            {
              if (num10 - 1 >= 0 && !DiscreteShadowCaster.DoesOcclude(num2, num10 - 1) && !DiscreteShadowCaster.DoesOcclude(num2 + 1, num10 - 1))
                DiscreteShadowCaster.ScanOctant(cellPos, range, depth + 1, octant, startSlope, DiscreteShadowCaster.GetSlope((double) num2 + 0.5, (double) num10 - 0.5, (double) cellPos.x, (double) cellPos.y, true), visiblePoints);
            }
            else
            {
              if (num10 - 1 >= 0 && DiscreteShadowCaster.DoesOcclude(num2, num10 - 1))
                startSlope = DiscreteShadowCaster.GetSlope((double) num2 - 0.5, (double) num10 - 0.5, (double) cellPos.x, (double) cellPos.y, true);
              if (!DiscreteShadowCaster.DoesOcclude(num2 + 1, num10) && !visiblePoints.Contains(Grid.XYToCell(num2, num10)))
      ...
      case DiscreteShadowCaster.Octant.W_NW:
        ...
        for (; DiscreteShadowCaster.GetSlope((double) num2, (double) num11, (double) cellPos.x, (double) cellPos.y, true) <= endSlope; --num11)
        {
          ...
          {
            if (DiscreteShadowCaster.DoesOcclude(num2, num11))
            {
              if (num11 + 1 < Grid.HeightInCells && !DiscreteShadowCaster.DoesOcclude(num2, num11 + 1) && !DiscreteShadowCaster.DoesOcclude(num2 + 1, num11 + 1))
                DiscreteShadowCaster.ScanOctant(cellPos, range, depth + 1, octant, startSlope, DiscreteShadowCaster.GetSlope((double) num2 + 0.5, (double) num11 + 0.5, (double) cellPos.x, (double) cellPos.y, true), visiblePoints);
            }
            else
            {
              if (num11 + 1 < Grid.HeightInCells && DiscreteShadowCaster.DoesOcclude(num2, num11 + 1))
                startSlope = -DiscreteShadowCaster.GetSlope((double) num2 - 0.5, (double) num11 + 0.5, (double) cellPos.x, (double) cellPos.y, true);
              if (!DiscreteShadowCaster.DoesOcclude(num2 + 1, num11) && !visiblePoints.Contains(Grid.XYToCell(num2, num11)))
