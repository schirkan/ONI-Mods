...
public class MinionModifiers : Modifiers, ISaveLoadable
{
  ...
  private AmountInstance AddAmount(Amount amount) => this.amounts.Add(new AmountInstance(amount, this.gameObject));
  ...
  private void OnAttachFollowCam(object data) => this.GetComponent<Effects>().Add("CenterOfAttention", false);

  private void OnDetachFollowCam(object data) => this.GetComponent<Effects>().Remove("CenterOfAttention");
}
