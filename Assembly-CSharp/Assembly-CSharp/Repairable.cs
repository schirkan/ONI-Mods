...
public class Repairable : Workable
{
  ...
  protected override bool OnWorkTick(Worker worker, float dt)
  {
    ...
    float num2 = (float) (((double) this.expectedRepairTime < 0.0 ? (double) num1 : (double) this.expectedRepairTime) * 0.100000001490116);
  ...
  public class SMInstance : GameStateMachine<Repairable.States, Repairable.SMInstance, Repairable, object>.GameInstance
  {
    ...
    public void DestroyStorageProxy()
    {
      ...
      this.smi.master.storageProxy.DropAll();
  ...
  public class States : GameStateMachine<Repairable.States, Repairable.SMInstance, Repairable>
  {
    ...
    private Chore CreateFetchChore(Repairable.SMInstance smi)
    {
      ...
      float amount = (float) ((double) component.Mass * 0.100000001490116 - ((UnityEngine.Object) primaryElement != (UnityEngine.Object) null ? (double) primaryElement.Mass : 0.0));
      Tag[] tags = new Tag[1]
      {
        GameTagExtensions.Create(component.ElementID)
      };
      return (Chore) new FetchChore(Db.Get().ChoreTypes.RepairFetch, smi.master.storageProxy, amount, tags, operational_requirement: FetchOrder2.OperationalRequirement.None);
    }

    private Chore CreateRepairChore(Repairable.SMInstance smi)
    {
      WorkChore<Repairable> workChore = new WorkChore<Repairable>(Db.Get().ChoreTypes.Repair, (IStateMachineTarget) smi.master, only_when_operational: false, ignore_building_assignment: true);
      ...
      workChore.AddPrecondition(Repairable.States.IsNotAngry);
