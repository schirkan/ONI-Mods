...
public class ResearchEntry : KMonoBehaviour
{
  ...
  public void SetTech(Tech newTech)
  {
    ...
    else
    {
      ...
      foreach (TechItem unlockedItem in this.targetTech.unlockedItems)
      {
        ...
        componentInChildrenOnly.GetComponent<ToolTip>().toolTip = str2;
  ...
  public void UpdateProgressBars()
  {
    foreach (KeyValuePair<string, GameObject> keyValuePair in this.progressBarsByResearchTypeID)
    {
      ...
      {
        num = 1f;
        child.GetComponentInChildren<LocText>().text = this.targetTech.costsByResearchTypeID[keyValuePair.Key].ToString() + "/" + (object) this.targetTech.costsByResearchTypeID[keyValuePair.Key];
      }
      else
      {
        ...
        if (orAdd != null)
        {
          child.GetComponentInChildren<LocText>().text = orAdd.progressInventory.PointsByTypeID[keyValuePair.Key].ToString() + "/" + (object) this.targetTech.costsByResearchTypeID[keyValuePair.Key];
  ...
  private GameObject GetFreeIcon() => Util.KInstantiateUI(this.iconPrefab, this.iconPanel);

  private Image GetFreeLine() => Util.KInstantiateUI<Image>(this.linePrefab.gameObject, this.gameObject);
