...
public class PathFinder
{
  ...
  public static void Initialize()
  {
    ...
    for (int index = 0; index < Grid.CellCount; ++index)
    {
      if (Grid.Visible[index] > (byte) 0 || Grid.Spawnable[index] > (byte) 0)
      {
        ...
        GameUtil.FloodFillConditional(index, PathFinder.allowPathfindingFloodFillCb, (ICollection<int>) pooledList);
        Grid.AllowPathfinding[index] = true;
        pooledList.Recycle();
      }
    }
    Grid.OnReveal += new System.Action<int>(PathFinder.OnReveal);
  }
    ...
    NavType parent_nav_type)
  {
    ...
    for (int index2 = 0; index2 < num2; ++index2)
    {
      ...
      int num3 = abilities.TraversePath(ref path, potential.cell, potential.navType, cost1, (int) link2.transitionId, underwater_cost1) ? 1 : 0;
  ...
  public struct PotentialPath
  {
    ...
    public bool HasAnyFlag(PathFinder.PotentialPath.Flags mask) => (uint) (this.flags & mask) > 0U;
