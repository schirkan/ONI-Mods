...
namespace ProcGen
{
  ...
  public class WorldLayout
  {
    ...
    {
      Debug.Assert(this.mapWidth != 0 && (uint) this.mapHeight > 0U, (object) "Map size has not been set");
      ...
      if (!this.worldGen.Settings.world.noStart)
      {
        ...
        node1 = this.overworldGraph.AddNode(startSubworldName);
      ...
      if (node1 != null)
        avoidPoints.Add(node1.position);
      ...
      for (int index = 0; index < randomPoints.Count; ++index)
      {
        ...
        this.voronoiTree.AddSite(new Diagram.Site((uint) node2.node.Id, node2.position), VoronoiTree.Node.NodeType.Internal).tags.Add(WorldGenTags.UnassignedNode);
        ...
        for (int childIndex = 0; childIndex < this.voronoiTree.ChildCount(); ++childIndex)
        {
          ...
          if (child.type == VoronoiTree.Node.NodeType.Internal)
          {
            ...
            for (int index = 0; index < neighborsByEdge.Count; ++index)
            {
              ...
              this.overworldGraph.GetCorner(keyValuePair.Value.p0.Value);
              this.overworldGraph.GetCorner(keyValuePair.Value.p1.Value);
            }
          }
        }
          ...
          if (child.type == VoronoiTree.Node.NodeType.Internal)
          {
            ...
            for (int index = 0; index < neighborsByEdge.Count; ++index)
            {
              ...
              if (key != null)
              {
                ...
                edge = this.overworldGraph.GetEdge(corner1, corner2, cell1, cell2, true, out bool _);
              ...
              {
                edge = this.overworldGraph.GetEdge(corner1, corner2, cell1, cell1, true, out bool _);
    ...
    public bool IsNodeBorderOpen(VoronoiTree.Node n1, VoronoiTree.Node n2, TagSet edgeOpenTags)
    {
      ...
      if (nodeById1.type == nodeById2.type || subWorld1.zoneType == subWorld2.zoneType || subWorld1.zoneType == SubWorld.ZoneType.Space && subWorld2.zoneType == SubWorld.ZoneType.Space)
        return true;
      return cell1.tags.ContainsOne(edgeOpenTags) && cell2.tags.ContainsOne(edgeOpenTags);
    }
    ...
    private void AddSubworldChildren()
    {
      ...
      TagSet tagSet2 = defaultMoveTags != null ? new TagSet((IEnumerable<string>) defaultMoveTags) : (TagSet) null;
      ...
      for (int childIndex1 = 0; childIndex1 < this.voronoiTree.ChildCount(); ++childIndex1)
      {
        ...
        if (child.type == VoronoiTree.Node.NodeType.Internal)
        {
          ...
          node.RelaxRecursive(0, 10, pd: (this.worldGen.Settings.world.layoutMethod == World.LayoutMethod.PowerTree));
          List<VoronoiTree.Node> nodes = new List<VoronoiTree.Node>();
      ...
      bool testAvoidPoints = true)
    {
      ...
      while (randomPoints.Count < minPointCount && num1 < 10);
      return randomPoints;
    }
      ...
      int seed)
    {
      ...
      int minPointCount = sw.features.Count > 0 ? sw.features.Count : 2;
      List<Vector2> points = this.GetPoints(sw.name, node.log, minPointCount, node.site.poly, valueWithinRange1, sw.avoidRadius, avoidPoints, sw.sampleBehaviour, true, rnd, testAvoidPoints: sw.doAvoidPoints);
      ...
      else
      {
        ...
        {
          Debug.Log((object) string.Format("Applying Moved Tag {0} to {1}", (object) tagSet2[i].Name, (object) node.site.id));
      ...
      int i)
    {
      ...
      VoronoiTree.Node node2 = node.AddSite(new Diagram.Site((uint) node1.node.Id, node1.position), VoronoiTree.Node.NodeType.Internal);
    ...
    private void SplitFunction(Tree tree, VoronoiTree.Node.SplitCommand cmd)
    {
      ...
      if (originalTags.Contains(WorldGenTags.Feature))
      {
        ...
        node2.SetPosition(originalTags.Contains(WorldGenTags.CenteralFeature) ? tree.site.poly.Centroid() : tree.site.position);
      ...
      List<Vector2> points = this.GetPoints(tree.site.id.ToString(), tree.log, cmd.minChildCount, tree.site.poly, density, 1f, avoidPoints, PointGenerator.SampleBehaviour.PoissonDisk, true, tree.myRandom);
      ...
      for (int index = 0; index < points.Count; ++index)
      {
        Node node2 = this.worldGen.WorldLayout.localGraph.AddNode(cmd.typeOverride == null ? node1.type : cmd.typeOverride(points[index]));
    ...
    private void SprinklePOI(List<TemplateContainer> poi)
    {
      ...
      leafNodesWithTag.RemoveAll((Predicate<VoronoiTree.Node>) (vn => vn.tags.Contains(WorldGenTags.AtDepths) || vn.tags.Contains(WorldGenTags.AtSurface)));
    ...
    {
      List<VoronoiTree.Node> nodes = new List<VoronoiTree.Node>();
      this.voronoiTree.GetLeafNodes(nodes, (Tree.LeafNodeTest) (node => node.tags != null && node.tags.Contains(tag)));
      return nodes;
    }
    ...
    private Node FindFirstNode(string nodeType) => this.localGraph.FindNode((Predicate<Node>) (node => node.type == nodeType));

    private Node FindFirstNodeWithTag(Tag tag) => this.localGraph.FindNode((Predicate<Node>) (node => node.tags != null && node.tags.Contains(tag)));

    public Vector2I GetStartLocation()
    {
      if (this.worldGen.Settings.world.noStart)
      {
        Debug.Log((object) "World is configured 'noStart'");
    ...
    public List<River> GetRivers()
    {
      ...
      foreach (Satsuma.Arc arc in this.localGraph.baseGraph.Arcs(ArcFilter.All))
      {
        ...
        if (tn0 != null && tn1 != null && !(tn0.type != tn1.type) && (tn0.type.Contains(WorldGenTags.River.Name) && riverList.Find((Predicate<River>) (r => r.SinkPosition() == tn0.position && r.SourcePosition() == tn1.position)) == null))
        {
          River river;
