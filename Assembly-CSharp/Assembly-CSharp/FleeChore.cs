...
public class FleeChore : Chore<FleeChore.StatesInstance>
{
  ...
  private bool isInFavoredDirection(int cell, int fleeFromCell) => ((double) Grid.CellToPos(fleeFromCell).x < (double) this.gameObject.transform.GetPosition().x ? 1 : 0) == ((double) Grid.CellToPos(fleeFromCell).x < (double) Grid.CellToPos(cell).x ? (true ? 1 : 0) : (false ? 1 : 0));
