...
public class SteamUGCService : MonoBehaviour
{
  ...
  public void Awake()
  {
    ...
    if (numSubscribedItems == 0U)
      return;
    PublishedFileId_t[] pvecPublishedFileID = new PublishedFileId_t[(int) numSubscribedItems];
  ...
  public bool IsSubscribed(PublishedFileId_t item) => this.downloads.Contains(item) || this.proxies.Contains(item) || (this.queries.Contains(item) || this.publishes.Any<SteamUGCDetails_t>((Func<SteamUGCDetails_t, bool>) (candidate => candidate.m_nPublishedFileId == item))) || this.mods.Exists((Predicate<SteamUGCService.Mod>) (candidate => candidate.fileId == item));

  public SteamUGCService.Mod FindMod(PublishedFileId_t item) => this.mods.Find((Predicate<SteamUGCService.Mod>) (candidate => candidate.fileId == item));
    ...
    if (details.m_hPreviewFile != UGCHandle_t.Invalid)
    {
      ...
      if (SteamRemoteStorage.UGCRead(details.m_hPreviewFile, numArray, details.m_nPreviewFileSize, 0U, EUGCReadAction.k_EUGCRead_ContinueReadingUntilFinished) != details.m_nPreviewFileSize)
      {
        if (this.retry_counts[details.m_nPublishedFileId] % 100 == 0)
          Debug.LogFormat("Preview image load failed", (object[]) Array.Empty<object>());
        numArray = (byte[]) null;
      }
    }
    else
  ...
  private void Update()
  {
    ...
    if (pooledList2.Count != 0 || pooledList3.Count != 0 || flag && this.removals.Count != 0 || loaded_previews.Count != 0)
    {
      foreach (SteamUGCService.IClient client in this.clients)
        client.UpdateMods((IEnumerable<PublishedFileId_t>) pooledList2, (IEnumerable<PublishedFileId_t>) pooledList3, flag ? (IEnumerable<PublishedFileId_t>) this.removals : Enumerable.Empty<PublishedFileId_t>(), (IEnumerable<SteamUGCService.Mod>) loaded_previews);
    }
    pooledList2.Recycle();
    pooledList3.Recycle();
  ...
  private void OnSteamUGCQueryDetailsCompleted(SteamUGCQueryCompleted_t pCallback, bool bIOFailure)
  {
    switch (pCallback.m_eResult)
    {
      ...
      case EResult.k_EResultBusy:
        ...
        break;
      default:
        Debug.Log((object) ("[OnSteamUGCQueryDetailsCompleted] - handle: " + (object) pCallback.m_handle + " -- Result: " + (object) pCallback.m_eResult + " -- NUm results: " + (object) pCallback.m_unNumResultsReturned + " --Total Matching: " + (object) pCallback.m_unTotalMatchingResults + " -- cached: " + pCallback.m_bCachedData.ToString()));
    ...
    bool getFirstMatch = false)
  {
    ...
    string pchFolder;
    SteamUGC.GetItemInstallInfo(item, out ulong _, out pchFolder, 1024U, out uint _);
    try
    {
