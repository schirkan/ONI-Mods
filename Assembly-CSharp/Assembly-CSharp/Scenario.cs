...
public class Scenario : KMonoBehaviour
{
  ...
  private void OnWorldGenComplete() => this.Init();
  ...
  private void RunAfterNextUpdate(System.Action action) => this.StartCoroutine(this.RunAfterNextUpdateRoutine(action));
  ...
  public void SetupElementTest()
  {
    ...
    foreach (Element element1 in ElementLoader.elements.Where<Element>((Func<Element, bool>) (element => element.IsLiquid && !elements.Contains(element))).OrderBy<Element, SimHashes>((Func<Element, SimHashes>) (element => element.highTempTransitionTarget)).ToList<Element>())
    {
      Element element2 = element1;
      int left = builder1.Left;
      bool hasTransitionUp;
        ...
        else
        {
          ...
          for (int y = this.Min.y; y < this.Max.y; ++y)
          {
            ...
            if (!this.Scenario.ReplaceElementMask[gameCell])
              SimMessages.ReplaceElement(gameCell, element, CellEventLogger.Instance.Scenario, (float) num);
          }
        }
