...
public class ConsumablesTableScreen : TableScreen
{
    ...
    ToolTip tooltip)
  {
    ...
    switch (widgetRow.rowType)
    {
      case TableRow.RowType.Header:
        ...
        if (consumableInfo != null)
        {
          tooltip.AddMultiStringTooltip(string.Format((string) STRINGS.UI.CONSUMABLESSCREEN.FOOD_AVAILABLE, (object) GameUtil.GetFormattedCalories(WorldInventory.Instance.GetAmount(widgetColumn.consumable_info.ConsumableId.ToTag()) * consumableInfo.CaloriesPerUnit)), (ScriptableObject) null);
        ...
        tooltip.AddMultiStringTooltip(string.Format((string) STRINGS.UI.CONSUMABLESSCREEN.FOOD_AVAILABLE, (object) GameUtil.GetFormattedUnits(WorldInventory.Instance.GetAmount(widgetColumn.consumable_info.ConsumableId.ToTag()))), (ScriptableObject) null);
        break;
      case TableRow.RowType.Default:
    ...
    else
    {
      ...
      switch (widgetRow.rowType)
      {
        case TableRow.RowType.Header:
          ...
          reference.material = (double) WorldInventory.Instance.GetAmount(consumableInfo.ConsumableId.ToTag()) > 0.0 ? Assets.UIPrefabs.TableScreenWidgets.DefaultUIMaterial : Assets.UIPrefabs.TableScreenWidgets.DesaturatedUIMaterial;
          break;
        case TableRow.RowType.Default:
    ...
    GameObject widget_go)
  {
    ...
    switch (widgetRow.rowType)
    {
      case TableRow.RowType.Header:
        ...
        break;
      case TableRow.RowType.Default:
        resultValues = ConsumerManager.instance.DefaultForbiddenTagsList.Contains(consumableInfo.ConsumableId.ToTag()) ? TableScreen.ResultValues.False : TableScreen.ResultValues.True;
        break;
      case TableRow.RowType.Minion:
        resultValues = !((UnityEngine.Object) (minion as MinionIdentity) != (UnityEngine.Object) null) ? TableScreen.ResultValues.True : (((Component) minion).GetComponent<ConsumableConsumer>().IsPermitted(consumableInfo.ConsumableId) ? TableScreen.ResultValues.True : TableScreen.ResultValues.False);
        break;
      case TableRow.RowType.StoredMinon:
        resultValues = !((UnityEngine.Object) (minion as StoredMinionIdentity) != (UnityEngine.Object) null) ? TableScreen.ResultValues.True : (((StoredMinionIdentity) minion).IsPermittedToConsume(consumableInfo.ConsumableId) ? TableScreen.ResultValues.True : TableScreen.ResultValues.False);
        break;
    }
    return resultValues;
  }
  ...
  {
    ConsumableInfoTableColumn consumableInfoTableColumn = new ConsumableInfoTableColumn(consumable_info, load_value_action, get_value_action, on_press_action, set_value_action, sort_comparison, on_tooltip, on_sort_tooltip, (Func<GameObject, string>) (widget_go => ""));
    consumableInfoTableColumn.scrollerID = "consumableScroller";
    return this.RegisterColumn(id, (TableColumn) consumableInfoTableColumn) ? consumableInfoTableColumn : (ConsumableInfoTableColumn) null;
  }
