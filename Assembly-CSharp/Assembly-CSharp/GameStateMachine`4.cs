  ...
  where MasterType : IStateMachineTarget
{
  ...
  {
    public void Queue(string anim, KAnim.PlayMode mode = KAnim.PlayMode.Once) => this.smi.GetComponent<KBatchedAnimController>().Queue((HashedString) anim, mode);

    public void Play(string anim, KAnim.PlayMode mode = KAnim.PlayMode.Once) => this.smi.GetComponent<KBatchedAnimController>().Play((HashedString) anim, mode);
  ...
  public new class State : StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State
  {
      ...
      StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.FloatParameter actual_amount)
    {
      ...
      this.Enter("Reserve(" + pickup_target.name + ", " + requested_amount.name + ")", (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State.Callback) (smi =>
      {
        ...
        if ((double) amount <= 0.0)
        {
          ...
          Debug.LogError((object) (val2.ToString() + ", " + (object) val1 + ", " + (object) pickupable.UnreservedAmount + ", " + (object) amount));
        }
      ...
      GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State failure_state)
    {
      ...
      this.ToggleWork("Eat", (System.Action<StateMachineInstanceType>) (smi =>
      {
        ...
        Edible.EdibleStartWorkInfo edibleStartWorkInfo = new Edible.EdibleStartWorkInfo((Workable) edible, amount1);
        worker.StartWork((Worker.StartWorkInfo) edibleStartWorkInfo);
      ...
      GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State failure_state)
    {
      ...
      this.ToggleWork("Pickup", (System.Action<StateMachineInstanceType>) (smi =>
      {
        ...
        Worker worker = state_target.Get<Worker>(smi);
        float amount1 = amount.Get(smi);
        Pickupable.PickupableStartWorkInfo pickupableStartWorkInfo = new Pickupable.PickupableStartWorkInfo(pickupable, amount1, (System.Action<GameObject>) (result => result_target.Set(result, smi)));
        worker.StartWork((Worker.StartWorkInfo) pickupableStartWorkInfo);
      ...
      Func<StateMachineInstanceType, Notification> callback)
    {
      ...
      this.Enter("EnableNotification()", (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State.Callback) (smi =>
      {
        ...
        smi.master.gameObject.AddOrGet<Notifier>().Add(notification);
      }));
    ...
    public GameStateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State StopMoving()
    {
      ...
      this.Exit("StopMoving()", (StateMachine<StateMachineType, StateMachineInstanceType, MasterType, DefType>.State.Callback) (smi => target.Get<Navigator>(smi).Stop()));
      return this;
    }
