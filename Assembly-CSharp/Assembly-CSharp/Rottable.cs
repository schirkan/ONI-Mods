...
public class Rottable : GameStateMachine<Rottable, Rottable.Instance, IStateMachineTarget, Rottable.Def>
{
  ...
  {
    selectable.SetStatusItem(Db.Get().StatusItemCategories.PreservationTemperature, refrigerated ? Db.Get().CreatureStatusItems.Refrigerated : Db.Get().CreatureStatusItems.Unrefrigerated, (object) selectable);
    switch (atmoshpere)
    {
      case Rottable.RotAtmosphereQuality.Normal:
        selectable.SetStatusItem(Db.Get().StatusItemCategories.PreservationAtmosphere, (StatusItem) null);
        break;
  ...
  public new class Instance : GameStateMachine<Rottable, Rottable.Instance, IStateMachineTarget, Rottable.Def>.GameInstance
  {
      ...
      : base(master, def)
    {
      ...
      this.UnrefrigeratedModifier = new AttributeModifier("Rot", 0.0f, (string) DUPLICANTS.MODIFIERS.ROTTEMPERATURE.NAME, is_readonly: false);
      this.ContaminatedAtmosphere = new AttributeModifier("Rot", 0.0f, (string) DUPLICANTS.MODIFIERS.ROTATMOSPHERE.NAME, is_readonly: false);
    ...
    public void RefreshModifiers(float dt)
    {
      ...
      Rottable.RotAtmosphereQuality atmosphereQuality = (double) this.ContaminatedAtmosphere.Value != 0.0 ? ((double) this.ContaminatedAtmosphere.Value > 0.0 ? Rottable.RotAtmosphereQuality.Sterilizing : Rottable.RotAtmosphereQuality.Contaminating) : Rottable.RotAtmosphereQuality.Normal;
      int num1 = (double) this.UnrefrigeratedModifier.Value == 0.0 ? 1 : 0;
      int num2 = (int) atmosphereQuality;
      Rottable.SetStatusItems(component, num1 != 0, (Rottable.RotAtmosphereQuality) num2);
    ...
    private float rotTemperatureModifier() => !Rottable.IsRefrigerated(this.gameObject) ? -0.5f : 0.0f;
    ...
    public bool IsRotLevelStackable(Rottable.Instance other) => (double) Mathf.Abs(this.RotConstitutionPercentage - other.RotConstitutionPercentage) < 0.100000001490116;
