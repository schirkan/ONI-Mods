...
public class StructureTemperatureComponents : KGameObjectSplitComponentManager<StructureTemperatureHeader, StructureTemperaturePayload>
{
    ...
    ref StructureTemperaturePayload payload)
  {
    ...
    float operatingKilowatts = payload.OperatingKilowatts;
    Extents extents = payload.GetExtents();
    byte num1 = (byte) ElementLoader.elements.IndexOf(primaryElement.Element);
    double num2 = (double) temperatureModification;
    double num3 = (double) internalTemperature;
    double thermalConductivity = (double) def.ThermalConductivity;
    double num4 = (double) operatingKilowatts;
    int num5 = (int) num1;
    int index = handle1.index;
    SimMessages.AddBuildingHeatExchange(extents, (float) num2, (float) num3, (float) thermalConductivity, (float) num4, (byte) num5, index);
