...
public class CameraController : KMonoBehaviour, IInputHandler
{
  ...
  public void SetWorldInteractive(bool state) => GameScreenManager.Instance.fadePlane.raycastTarget = !state;

  private IEnumerator FadeToBlack(float targetBlackPercent = 1f, float speed = 1f)
  {
    Mathf.Max(0.0f, GameScreenManager.Instance.fadePlane.color.a);
    float duration = 1f;
    for (float i = 0.0f; (double) i <= (double) duration; i += Time.unscaledDeltaTime * speed)
    {
      GameScreenManager.Instance.fadePlane.color = new Color(0.0f, 0.0f, 0.0f, Mathf.Max(Mathf.Min(i / duration, targetBlackPercent), GameScreenManager.Instance.fadePlane.color.a));
      yield return (object) 0;
    }
    GameScreenManager.Instance.fadePlane.color = new Color(0.0f, 0.0f, 0.0f, targetBlackPercent);
  ...
  private IEnumerator FadeInFromBlack(float targetBlackPercent = 0.0f, float speed = 1f)
  {
    ...
    for (float i = 0.0f; (double) i <= (double) duration; i += Time.unscaledDeltaTime * speed)
    {
      GameScreenManager.Instance.fadePlane.color = new Color(0.0f, 0.0f, 0.0f, Mathf.Min(Mathf.Max((float) (1.0 - (double) i / (double) duration), targetBlackPercent), GameScreenManager.Instance.fadePlane.color.a));
      yield return (object) 0;
    }
    GameScreenManager.Instance.fadePlane.color = new Color(0.0f, 0.0f, 0.0f, targetBlackPercent);
    this.activeFadeRoutine = (Coroutine) null;
  ...
  public void OnKeyDown(KButtonEvent e)
  {
    ...
    {
      this.targetOrthographicSize = Mathf.Min(this.targetOrthographicSize + this.zoomFactor * this.targetOrthographicSize, this.FreeCameraEnabled ? TuningData<CameraController.Tuning>.Get().maxOrthographicSizeDebug : this.maxOrthographicSize);
      this.overrideZoomSpeed = 0.0f;
      this.isTargetPosSet = false;
    }
  ...
  public bool IsAudibleSound(Vector3 pos, HashedString sound_path) => this.soundCuller.IsAudible((Vector2) pos, sound_path);
  ...
  public void UpdateFollowTarget()
  {
    if (!((UnityEngine.Object) this.followTarget != (UnityEngine.Object) null))
      return;
    Vector3 followPos = this.GetFollowPos();
    Vector2 vector2 = Vector2.Lerp(new Vector2(this.transform.GetLocalPosition().x, this.transform.GetLocalPosition().y), (Vector2) followPos, Time.unscaledDeltaTime * 25f);
