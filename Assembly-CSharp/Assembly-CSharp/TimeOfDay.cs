...
public class TimeOfDay : KMonoBehaviour, ISaveLoadable
{
  ...
  public TimeOfDay.TimeRegion GetCurrentTimeRegion() => (double) GameClock.Instance.GetCurrentCycleAsPercentage() >= 0.875 ? TimeOfDay.TimeRegion.Night : TimeOfDay.TimeRegion.Day;
  ...
  private void TriggerSoundChange(TimeOfDay.TimeRegion new_region)
  {
    switch (new_region)
    {
      case TimeOfDay.TimeRegion.Day:
        AudioMixer.instance.Stop((HashedString) AudioMixerSnapshots.Get().NightStartedMigrated);
