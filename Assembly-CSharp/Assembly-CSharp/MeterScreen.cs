...
public class MeterScreen : KScreen, IRender1000ms
{
  ...
  private void RefreshSick() => this.SickText.text = MeterScreen.CountSickDupes().ToString();
  ...
  private void RefreshRations()
  {
    if (!((UnityEngine.Object) this.RationsText != (UnityEngine.Object) null) || !((UnityEngine.Object) RationTracker.Get() != (UnityEngine.Object) null))
      return;
    long num = (long) RationTracker.Get().CountRations((Dictionary<string, float>) null);
    if (this.cachedCalories == num)
      return;
    this.RationsText.text = GameUtil.GetFormattedCalories((float) num);
  ...
  private string OnSickTooltip()
  {
    ...
    for (int idx = 0; idx < Components.LiveMinionIdentities.Count; ++idx)
    {
      ...
      bool selected = idx == this.immunityDisplayInfo.selectedIndex;
  ...
  private string OnRationsTooltip()
  {
    ...
    float calories = RationTracker.Get().CountRations(this.rationsDict);
    this.RationsText.text = GameUtil.GetFormattedCalories(calories);
    this.RationsTooltip.ClearMultiStringTooltip();
    this.RationsTooltip.AddMultiStringTooltip(string.Format((string) UI.TOOLTIPS.METERSCREEN_MEALHISTORY, (object) GameUtil.GetFormattedCalories(calories)), (ScriptableObject) this.ToolTipStyle_Header);
    this.RationsTooltip.AddMultiStringTooltip("", (ScriptableObject) this.ToolTipStyle_Property);
    ...
    })).ToDictionary<KeyValuePair<string, float>, string, float>((Func<KeyValuePair<string, float>, string>) (t => t.Key), (Func<KeyValuePair<string, float>, float>) (t => t.Value)))
    {
      ...
      this.RationsTooltip.AddMultiStringTooltip(foodInfo != null ? string.Format("{0}: {1}", (object) foodInfo.Name, (object) GameUtil.GetFormattedCalories(keyValuePair.Value * foodInfo.CaloriesPerUnit)) : string.Format((string) UI.TOOLTIPS.METERSCREEN_INVALID_FOOD_TYPE, (object) keyValuePair.Key), (ScriptableObject) this.ToolTipStyle_Property);
    }
    return "";
  }
  ...
  private void RefreshStress() => this.StressText.text = Mathf.Round(GameUtil.GetMaxStress()).ToString();
  ...
  private IList<MinionIdentity> GetSickMinions() => (IList<MinionIdentity>) Components.LiveMinionIdentities.Items;
