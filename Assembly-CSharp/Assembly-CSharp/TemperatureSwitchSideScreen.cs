...
public class TemperatureSwitchSideScreen : SideScreenContent, IRender200ms
{
  ...
  public override bool IsValidForTarget(GameObject target) => (UnityEngine.Object) target.GetComponent<TemperatureControlledSwitch>() != (UnityEngine.Object) null;
  ...
  private void UpdateTargetTemperatureLabel() => this.targetTemperature.text = GameUtil.GetFormattedTemperature(this.targetTemperatureSwitch.thresholdTemperature);

  private void UpdateLabels() => this.currentTemperature.text = string.Format((string) STRINGS.UI.UISIDESCREENS.TEMPERATURESWITCHSIDESCREEN.CURRENT_TEMPERATURE, (object) GameUtil.GetFormattedTemperature(this.targetTemperatureSwitch.GetTemperature()));
}
