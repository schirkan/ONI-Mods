...
public class FilteredStorage
{
    ...
    ChoreType fetch_chore_type)
  {
    ...
    {
      FilteredStorage.capacityStatusItem = new StatusItem("StorageLocker", "BUILDING", "", StatusItem.IconType.Info, NotificationType.Neutral, false, OverlayModes.None.ID);
      ...
      FilteredStorage.noFilterStatusItem = new StatusItem("NoStorageFilterSet", "BUILDING", "status_item_no_filter_set", StatusItem.IconType.Custom, NotificationType.BadMinor, false, OverlayModes.None.ID);
    }
  ...
  private void CreateLogicMeter()
  {
    if (!this.hasMeter)
      return;
    this.logicMeter = new MeterController((KAnimControllerBase) this.root.GetComponent<KBatchedAnimController>(), "logicmeter_target", "logicmeter", Meter.Offset.Infront, Grid.SceneLayer.NoLayer, (string[]) Array.Empty<string>());
  }
  ...
  private void OnFilterChanged(Tag[] tags)
  {
    ...
    bool flag = tags != null && (uint) tags.Length > 0U;
    Color32 color32 = flag ? this.filterTint : this.noFilterTint;
    component.TintColour = color32;
    if (this.fetchList != null)
    {
