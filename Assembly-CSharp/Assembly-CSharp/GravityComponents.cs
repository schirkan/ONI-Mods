  ...
  public override void FixedUpdate(float dt)
  {
    ...
    for (int index = 0; index < this.data.Count; ++index)
    {
      ...
      if ((double) gravityComponent.elapsedTime >= 0.0 && !((UnityEngine.Object) gravityComponent.transform == (UnityEngine.Object) null))
      {
        ...
        if (!flag1 & flag2)
        {
          KBatchedAnimController effect = FXHelpers.CreateEffect("splash_step_kanim", new Vector3(vector2_2.x, vector2_2.y, 0.0f) + new Vector3(-0.38f, 0.75f, -0.1f), layer: Grid.SceneLayer.FXFront);
        ...
        if (Grid.IsValidCell(cell2))
        {
          if ((double) vector2_1.sqrMagnitude > 0.200000002980232 && Grid.IsValidCell(cell1) && (!Grid.Element[cell1].IsLiquid && Grid.Element[cell2].IsLiquid))
          {
            ...
            {
              string sound = Sounds.Instance.OreSplashSoundsMigrated[(int) ambience];
              if ((UnityEngine.Object) CameraController.Instance != (UnityEngine.Object) null && CameraController.Instance.IsAudibleSound((Vector3) vector2_2, (HashedString) sound))
                SoundEvent.PlayOneShot(sound, (Vector3) vector2_2);
            }
          }
          ...
          if (!flag3 && gravityComponent.landOnFakeFloors && Grid.FakeFloor[cell2])
          {
            ...
            {
              flag3 = component.NavGrid.NavTable.IsValid(cell2);
              if (!flag3)
              {
