...
public class SpacecraftManager : KMonoBehaviour, ISim1000ms
{
  ...
  private void GenerateRandomDestinations()
  {
    ...
    {
      int index2 = random.Next(0, intList.Count - 1);
      int num3 = intList[index2];
      intList.RemoveAt(index2);
      List<string> stringList = stringListList[num3];
      this.destinations.Add(new SpaceDestination(this.destinations.Count, stringList[random.Next(0, stringList.Count)], num3));
    }
  ...
  public int GetSpacecraftID(LaunchableRocket rocket)
  {
    foreach (Spacecraft spacecraft in this.spacecraft)
    {
      if ((UnityEngine.Object) spacecraft.launchConditions.gameObject == (UnityEngine.Object) rocket.gameObject)
  ...
  public Spacecraft GetSpacecraftFromLaunchConditionManager(LaunchConditionManager lcm)
  {
    foreach (Spacecraft spacecraft in this.spacecraft)
    {
      if ((UnityEngine.Object) spacecraft.launchConditions == (UnityEngine.Object) lcm)
        return spacecraft;
    }
  ...
  {
    Notification notification1 = new Notification((string) BUILDING.STATUSITEMS.SPACECRAFTREADYTOLAND.NOTIFICATION, NotificationType.Good, HashedString.Invalid, (Func<List<Notification>, object, string>) ((notificationList, data) =>
    {
      string str = (string) BUILDING.STATUSITEMS.SPACECRAFTREADYTOLAND.NOTIFICATION_TOOLTIP;
