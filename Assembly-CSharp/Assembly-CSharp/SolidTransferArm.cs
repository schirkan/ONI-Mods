...
public class SolidTransferArm : StateMachineComponent<SolidTransferArm.SMInstance>, ISim1000ms, IRenderEveryTick
{
  ...
  protected override void OnSpawn()
  {
    ...
    Vector3 column = (Vector3) component.GetSymbolTransform(new HashedString("arm_target"), out bool _).GetColumn(3);
    ...
    float time_delta)
  {
    ...
    else
    {
      ...
      for (int index = 0; index != num2; ++index)
      {
        ...
        int end = index == num2 - 1 ? shared_data.solid_transfer_arms.Count : start + num1;
  ...
  private void Sim()
  {
    ...
    if (this.choreConsumer.FindNextChore(ref out_context))
    {
      if (out_context.chore is FetchChore)
      {
        ...
        this.storage.DropUnlessHasTags(chore.tagBits, chore.requiredTagBits, chore.forbiddenTagBits);
