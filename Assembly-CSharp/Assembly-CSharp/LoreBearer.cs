...
public class LoreBearer : KMonoBehaviour
{
  ...
  public string content => (string) Strings.Get("STRINGS.LORE.BUILDINGS." + this.gameObject.name + ".ENTRY");
  ...
  private void RefreshUserMenu(object data = null) => Game.Instance.userMenu.AddButton(this.gameObject, new KIconButtonMenu.ButtonInfo("action_follow_cam", (string) UI.USERMENUACTIONS.READLORE.NAME, new System.Action(this.OnClickRead), tooltipText: ((string) UI.USERMENUACTIONS.READLORE.TOOLTIP)));

  private System.Action<InfoDialogScreen> OpenCodex(string key) => (System.Action<InfoDialogScreen>) (dialog =>
  {
    dialog.Deactivate();
