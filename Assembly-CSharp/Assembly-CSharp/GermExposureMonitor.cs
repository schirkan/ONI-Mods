...
public class GermExposureMonitor : GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance>
{
  ...
  public new class Instance : GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
      ...
      Sickness.InfectionVector vector)
    {
      foreach (ExposureType exposureType in GERM_EXPOSURE.TYPES)
      {
        if (disease.id == (HashedString) exposureType.germ_id && count > exposureType.exposure_threshold && (this.HasMinExposurePeriodElapsed(exposureType.germ_id) && this.IsExposureValidForTraits(exposureType)))
        {
      ...
      if (this.lastDiseaseSources.TryGetValue((HashedString) id, out diseaseSourceInfo))
      {
        switch (diseaseSourceInfo.vector)
        {
          case Sickness.InfectionVector.Contact:
            str = (string) DUPLICANTS.DISEASES.INFECTIONSOURCES.SKIN;
            break;
          case Sickness.InfectionVector.Digestion:
            str = string.Format((string) DUPLICANTS.DISEASES.INFECTIONSOURCES.FOOD, (object) diseaseSourceInfo.sourceObject.ProperName());
            break;
          case Sickness.InfectionVector.Inhalation:
            str = string.Format((string) DUPLICANTS.DISEASES.INFECTIONSOURCES.AIR, (object) diseaseSourceInfo.sourceObject.ProperName());
            break;
          default:
            str = (string) DUPLICANTS.DISEASES.INFECTIONSOURCES.UNKNOWN;
            break;
        }
      }
      else
