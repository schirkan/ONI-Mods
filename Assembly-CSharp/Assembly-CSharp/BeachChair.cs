...
public class BeachChair : StateMachineComponent<BeachChair.StatesInstance>, IGameObjectEffectDescriptor
{
    ...
    bool high_lux)
  {
    ...
    {
      Descriptor descriptor = new Descriptor(locString1.Replace("{attrib}", (string) Strings.Get("STRINGS.DUPLICANTS.ATTRIBUTES." + selfModifier.AttributeId.ToUpper() + ".NAME")).Replace("{amount}", selfModifier.GetFormattedString((GameObject) null)).Replace("{lux}", GameUtil.GetFormattedLux(10000)), locString2.Replace("{attrib}", (string) Strings.Get("STRINGS.DUPLICANTS.ATTRIBUTES." + selfModifier.AttributeId.ToUpper() + ".NAME")).Replace("{amount}", selfModifier.GetFormattedString((GameObject) null)).Replace("{lux}", GameUtil.GetFormattedLux(10000)));
  ...
  public class States : GameStateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.ready.working_lit.silly.ParamTransition<bool>((StateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.Parameter<bool>) this.lit, this.ready.working_lit.post, GameStateMachine<BeachChair.States, BeachChair.StatesInstance, BeachChair, object>.IsFalse).Target(this.worker).PlayAnim((Func<BeachChair.StatesInstance, string>) (smi => this.SILLY_ANIMS[UnityEngine.Random.Range(0, this.SILLY_ANIMS.Length)]), KAnim.PlayMode.Once).OnAnimQueueComplete(this.ready.working_lit.working);
    ...
    private Chore CreateChore(BeachChair.StatesInstance smi)
    {
      ...
      WorkChore<BeachChairWorkable> workChore = new WorkChore<BeachChairWorkable>(Db.Get().ChoreTypes.Relax, (IStateMachineTarget) component, allow_in_red_alert: false, schedule_block: Db.Get().ScheduleBlockTypes.Recreation, allow_prioritization: false, priority_class: PriorityScreen.PriorityClass.high);
