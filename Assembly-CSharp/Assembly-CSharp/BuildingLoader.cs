...
public class BuildingLoader : KMonoBehaviour
{
  ...
  private static bool Add2DComponents(
    ...
    int layer = -1)
  {
    bool required = def.AnimFiles != null && (uint) def.AnimFiles.Length > 0U;
    if (layer == -1)
  ...
  public GameObject CreateBuildingUnderConstruction(BuildingDef def)
  {
    ...
    Rotatable rotatable = BuildingLoader.UpdateComponentRequirement<Rotatable>(building, (uint) def.PermittedRotations > 0U);
  ...
  public GameObject CreateBuildingComplete(GameObject go, BuildingDef def)
  {
    ...
    Rotatable rotatable = BuildingLoader.UpdateComponentRequirement<Rotatable>(go, (uint) def.PermittedRotations > 0U);
    if ((bool) (Object) rotatable)
      rotatable.permittedRotations = def.PermittedRotations;
    if (def.Breakable)
    ...
    BuildingLoader.UpdateComponentRequirement<RequireOutputs>(go, (uint) def.OutputConduitType > 0U);
