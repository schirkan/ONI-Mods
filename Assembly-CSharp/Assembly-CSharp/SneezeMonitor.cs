...
public class SneezeMonitor : GameStateMachine<SneezeMonitor, SneezeMonitor.Instance>
{
  ...
  public new class Instance : GameStateMachine<SneezeMonitor, SneezeMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
    ...
    private void OnSneezyChange() => this.smi.sm.isSneezy.Set((double) Db.Get().Attributes.Sneezyness.Lookup(this.master.gameObject).GetTotalValue() > 0.0, this.smi);

    public Reactable GetReactable()
    {
      float min_reactor_time = this.NextSneezeInterval();
      return (Reactable) new SelfEmoteReactable(this.master.gameObject, (HashedString) "Sneeze", Db.Get().ChoreTypes.Cough, (HashedString) "anim_sneeze_kanim", min_reactor_time: min_reactor_time).AddStep(new EmoteReactable.EmoteStep()
      {
    ...
    private void TriggerDisurbance(GameObject go) => AcousticDisturbance.Emit((object) go, 3);
