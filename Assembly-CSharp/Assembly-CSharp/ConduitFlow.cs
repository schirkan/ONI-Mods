...
public class ConduitFlow : IConduitFlow
{
  ...
  private void OnUtilityNetworksRebuilt(IList<UtilityNetwork> networks, ICollection<int> root_nodes)
  {
    ...
    for (int index1 = 0; index1 != networks.Count; ++index1)
    {
      ...
      else
      {
        ...
        ConduitFlow.Network network2 = network1;
        networks1.Add(network2);
      }
    }
    this.build_network_job.Reset(this);
    foreach (ConduitFlow.Network network2 in this.networks)
      this.build_network_job.Add(new ConduitFlow.BuildNetworkTask(network2, this.soaInfo.NumEntries));
    ...
    Dictionary<int, ConduitFlow.Sink> sinks)
  {
    ...
    return !sinks.TryGetValue(grid_node.conduitIdx, out sink) || !((UnityEngine.Object) sink.consumer != (UnityEngine.Object) null) ? contents.movable_mass : Mathf.Max(0.0f, contents.movable_mass - sink.space_remaining);
  }
    ...
    if ((double) movableMass <= 0.0)
    {
      for (int index = 0; index != 4; ++index)
      {
        ...
        if ((permittedFlowDirections & flowDirections1) != ConduitFlow.FlowDirections.None)
        {
          ...
          if ((uint) (this.soaInfo.GetSrcFlowDirection(conduitFromDirection.idx) & ConduitFlow.Opposite(flowDirections1)) > 0U)
    ...
    else
    {
      for (int index = 0; index != 4; ++index)
      {
        ...
        if ((permittedFlowDirections & flowDirections1) != ConduitFlow.FlowDirections.None)
        {
          ...
          bool flag2 = (uint) ((ConduitFlow.FlowDirections) srcFlowDirection & ConduitFlow.Opposite(flowDirections1)) > 0U;
  ...
  private struct BuildNetworkTask : IWorkItem<ConduitFlow>
  {
    ...
    private void ComputeOrder(ConduitFlow outer)
    {
      ...
      while (this.distance_nodes.Count != 0)
      {
        ...
        if (conduitIdx != -1)
        {
          this.distances_via_sources[distanceNode2.cell] = distanceNode2.distance;
          ...
          if ((permittedFlowDirections & 8) != 0)
          {
            ...
            distanceNodes.Enqueue(distanceNode3);
          }
      ...
      foreach (int source in (HashSet<int>) this.from_sinks_graph.sources)
      {
        ...
        ConduitFlow.BuildNetworkTask.DistanceNode distanceNode2 = distanceNode1;
        distanceNodes.Enqueue(distanceNode2);
      }
      ...
      while (this.distance_nodes.Count != 0)
      {
        ...
        if (conduitIdx != -1)
        {
          ...
          ConduitFlow.ConduitConnections conduitConnections = outer.soaInfo.GetConduitConnections(conduitIdx);
          ConduitFlow.BuildNetworkTask.DistanceNode distanceNode3;
          ...
          {
            QueuePool<ConduitFlow.BuildNetworkTask.DistanceNode, ConduitFlow>.PooledQueue distanceNodes = this.distance_nodes;
            distanceNode3 = new ConduitFlow.BuildNetworkTask.DistanceNode();
            distanceNode3.cell = outer.soaInfo.GetCell(conduitConnections.up);
            distanceNode3.distance = distanceNode2.distance + 1;
            ConduitFlow.BuildNetworkTask.DistanceNode distanceNode4 = distanceNode3;
            distanceNodes.Enqueue(distanceNode4);
          }
          if (conduitConnections.down != -1 && (outer.soaInfo.GetPermittedFlowDirections(conduitConnections.down) & ConduitFlow.FlowDirections.Up) != ConduitFlow.FlowDirections.None)
          {
            QueuePool<ConduitFlow.BuildNetworkTask.DistanceNode, ConduitFlow>.PooledQueue distanceNodes = this.distance_nodes;
            distanceNode3 = new ConduitFlow.BuildNetworkTask.DistanceNode();
            distanceNode3.cell = outer.soaInfo.GetCell(conduitConnections.down);
            distanceNode3.distance = distanceNode2.distance + 1;
            ConduitFlow.BuildNetworkTask.DistanceNode distanceNode4 = distanceNode3;
            distanceNodes.Enqueue(distanceNode4);
          }
          if (conduitConnections.left != -1 && (outer.soaInfo.GetPermittedFlowDirections(conduitConnections.left) & ConduitFlow.FlowDirections.Right) != ConduitFlow.FlowDirections.None)
          {
            QueuePool<ConduitFlow.BuildNetworkTask.DistanceNode, ConduitFlow>.PooledQueue distanceNodes = this.distance_nodes;
            distanceNode3 = new ConduitFlow.BuildNetworkTask.DistanceNode();
            distanceNode3.cell = outer.soaInfo.GetCell(conduitConnections.left);
            distanceNode3.distance = distanceNode2.distance + 1;
            ConduitFlow.BuildNetworkTask.DistanceNode distanceNode4 = distanceNode3;
            distanceNodes.Enqueue(distanceNode4);
          }
          if (conduitConnections.right != -1 && (outer.soaInfo.GetPermittedFlowDirections(conduitConnections.right) & ConduitFlow.FlowDirections.Left) != ConduitFlow.FlowDirections.None)
          {
            QueuePool<ConduitFlow.BuildNetworkTask.DistanceNode, ConduitFlow>.PooledQueue distanceNodes = this.distance_nodes;
            distanceNode3 = new ConduitFlow.BuildNetworkTask.DistanceNode();
            distanceNode3.cell = outer.soaInfo.GetCell(conduitConnections.right);
            distanceNode3.distance = distanceNode2.distance + 1;
            ConduitFlow.BuildNetworkTask.DistanceNode distanceNode4 = distanceNode3;
            distanceNodes.Enqueue(distanceNode4);
          }
  ...
  private struct Sink
  {
    ...
    {
      this.consumer = (UnityEngine.Object) sink.GameObject != (UnityEngine.Object) null ? sink.GameObject.GetComponent<ConduitConsumer>() : (ConduitConsumer) null;
