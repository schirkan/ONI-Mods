...
public class WaterCoolerChore : Chore<WaterCoolerChore.StatesInstance>, IWorkerPrioritizable
{
  ...
  public class States : GameStateMachine<WaterCoolerChore.States, WaterCoolerChore.StatesInstance, WaterCoolerChore>
  {
    ...
    private void Drink(WaterCoolerChore.StatesInstance smi)
    {
      ...
      ref SimUtil.DiseaseInfo local1 = ref diseaseInfo;
      float num;
      ref float local2 = ref num;
      storage.ConsumeAndGetDisease(water, 1f, out local1, out local2);
      cmp.GetSMI<GermExposureMonitor.Instance>()?.TryInjectDisease(diseaseInfo.idx, diseaseInfo.count, GameTags.Water, Sickness.InfectionVector.Digestion);
