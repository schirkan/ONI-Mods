...
public class SolidConsumerMonitor : GameStateMachine<SolidConsumerMonitor, SolidConsumerMonitor.Instance, IStateMachineTarget, SolidConsumerMonitor.Def>
{
  ...
  public override void InitializeStates(out StateMachine.BaseState default_state)
  {
    ...
    this.lookingforfood.TagTransition(GameTags.Creatures.Hungry, this.satisfied, true).Update(new System.Action<SolidConsumerMonitor.Instance, float>(SolidConsumerMonitor.FindFood), UpdateRate.SIM_1000ms, true);
  }
  ...
  private static void FindFood(SolidConsumerMonitor.Instance smi, float dt)
  {
      ...
      foreach (Storage storage in (List<Storage>) pooledList2)
      {
        if (!((UnityEngine.Object) storage == (UnityEngine.Object) null))
        {
          foreach (GameObject gameObject in storage.items)
            pooledList1.Add((UnityEngine.Object) gameObject != (UnityEngine.Object) null ? gameObject.GetComponent<KMonoBehaviour>() : (KMonoBehaviour) null);
        }
      }
    }
    pooledList2.Recycle();
    ...
    GameScenePartitioner.Instance.GatherEntries(x_bottomLeft, y_bottomLeft, 16, 16, GameScenePartitioner.Instance.pickupablesLayer, (List<ScenePartitionerEntry>) pooledList3);
    GameScenePartitioner.Instance.GatherEntries(x_bottomLeft, y_bottomLeft, 16, 16, GameScenePartitioner.Instance.plants, (List<ScenePartitionerEntry>) pooledList3);
