...
public class WorldInspector : MonoBehaviour
{
  ...
  private void UpdateAsSimCell(CellSelectionObject cellObject)
  {
    ...
    this.TemperatureTextDisplay.text = GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(cellObject.temperature));
    this.Tooltip_CurrentTemperature.toolTip = "Current Temperature: " + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(cellObject.temperature));
  ...
  private void UpdateAsElementChunk(ElementChunk _chunkObject)
  {
    ...
    this.PropertyIcon_Right.sprite = this.propertySprites.Resource;
    this.TemperatureTextDisplay.text = GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(component.Temperature));
    string str = "Current Temperature: " + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(component.Temperature));
    this.SetStateColorScheme(0);
    this.Tooltip_CurrentTemperature.toolTip = str + this.SetCurrentTemperatureTooltip(ElementLoader.FindElementByHash(component.ElementID), 0) + "\nMelts at: <color=yellow>" + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(ElementLoader.FindElementByHash(component.ElementID).highTemp)) + "</color>";
    this.TemperatureNotch.SetActive(true);
  ...
  private void UpdateAsEdible(Edible edibleObject)
  {
    ...
    this.TemperatureTextDisplay.text = GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(f));
    string str = "Current Temperature: " + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(f));
    this.SetStateColorScheme(0);
    this.Tooltip_CurrentTemperature.toolTip = str + "\nRots at temperatures above: <color=yellow>" + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(edibleObject.FoodInfo.RotTemperature)) + "</color>";
    this.TemperatureNotch.SetActive(true);
  ...
  private string SetCurrentTemperatureTooltip(Element element, int state)
  {
    ...
    {
      case 0:
        str = str + "\nMelts at: <color=yellow>" + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(element.highTemp)) + "</color>";
        break;
      case 1:
        str = str + "\nFreezes at: <color=cyan>" + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(element.lowTemp)) + "</color>" + "\nEvaporates at: <color=red>" + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(element.highTemp)) + "</color>";
        break;
      case 2:
        str = str + "\nCondenses at: <color=yellow>" + GameUtil.GetFormattedTemperature((float) Mathf.RoundToInt(element.lowTemp)) + "</color>";
        break;
    }
    return str;
  }
  ...
  public static void DestroyStatics() => WorldInspector.cachedElement = (Element) null;
