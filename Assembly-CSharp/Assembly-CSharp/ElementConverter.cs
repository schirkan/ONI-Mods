...
public class ElementConverter : StateMachineComponent<ElementConverter.StatesInstance>, IGameObjectEffectDescriptor
{
  ...
  public bool HasEnoughMassToStartConverting() => this.HasEnoughMass();
      ...
      for (int index2 = 0; index2 < this.storage.items.Count; ++index2)
      {
        ...
        if (!((UnityEngine.Object) go == (UnityEngine.Object) null))
        {
          if (go.HasTag(consumedElement.tag))
          {
            ...
            int src2_count = (int) ((double) num6 / (double) component.Mass * (double) component.DiseaseCount);
      ...
      for (int index = 0; index < this.outputElements.Length; ++index)
      {
        ...
        if (outputElement.storeOutput)
        {
          ...
          if ((UnityEngine.Object) primaryElement == (UnityEngine.Object) null)
          {
            if (elementByHash.IsGas)
              this.storage.AddGasChunk(outputElement.elementHash, num6, temperature, a2.idx, a2.count, true);
            else if (elementByHash.IsLiquid)
              this.storage.AddLiquid(outputElement.elementHash, num6, temperature, a2.idx, a2.count, true);
            else
              this.storage.Store(elementByHash.substance.SpawnResource(this.transform.GetPosition(), num6, temperature, a2.idx, a2.count, true), true);
          }
          else
        ...
        else
        {
          ...
          if (elementByHash.IsLiquid)
          {
            ...
            FallingWater.instance.AddParticle(cell, (byte) idx, num6, temperature, a2.idx, a2.count, true);
          }
          else if (elementByHash.IsSolid)
            elementByHash.substance.SpawnResource(vector3, num6, temperature, a2.idx, a2.count);
          else
            SimMessages.AddRemoveSubstance(cell, outputElement.elementHash, CellEventLogger.Instance.OxygenModifierSimUpdate, num6, temperature, a2.idx, a2.count);
        }
        if (outputElement.elementHash == SimHashes.Oxygen)
          ReportManager.Instance.ReportValue(ReportManager.ReportType.OxygenCreated, num6, this.gameObject.GetProperName());
      }
    }
  ...
  public class StatesInstance : GameStateMachine<ElementConverter.States, ElementConverter.StatesInstance, ElementConverter, object>.GameInstance
  {
    ...
    public void RemoveStatusItems()
    {
      foreach (Guid statusItemEntry in this.statusItemEntries)
        this.master.GetComponent<KSelectable>().RemoveStatusItem(statusItemEntry);
  ...
  public class States : GameStateMachine<ElementConverter.States, ElementConverter.StatesInstance, ElementConverter>
  {
    ...
    public override void InitializeStates(out StateMachine.BaseState default_state)
    {
      ...
      this.disabled.EventTransition(GameHashes.ActiveChanged, this.converting, (StateMachine<ElementConverter.States, ElementConverter.StatesInstance, ElementConverter, object>.Transition.ConditionCallback) (smi => (UnityEngine.Object) smi.master.operational == (UnityEngine.Object) null || smi.master.operational.IsActive));
      this.converting.Enter("AddStatusItems", (StateMachine<ElementConverter.States, ElementConverter.StatesInstance, ElementConverter, object>.State.Callback) (smi => smi.AddStatusItems())).Exit("RemoveStatusItems", (StateMachine<ElementConverter.States, ElementConverter.StatesInstance, ElementConverter, object>.State.Callback) (smi => smi.RemoveStatusItems())).EventTransition(GameHashes.ActiveChanged, this.disabled, (StateMachine<ElementConverter.States, ElementConverter.StatesInstance, ElementConverter, object>.Transition.ConditionCallback) (smi => (UnityEngine.Object) smi.master.operational != (UnityEngine.Object) null && !smi.master.operational.IsActive)).Update("ConvertMass", (System.Action<ElementConverter.StatesInstance, float>) ((smi, dt) => smi.master.ConvertMass()), UpdateRate.SIM_1000ms, true);
    }
  }
}
