...
public static class TemplateCache
{
  ...
  public static string GetTemplatePath() => TemplateCache.baseTemplatePath;
  ...
  public static TemplateContainer GetTemplate(string templatePath)
  {
    ...
    if (TemplateCache.templates[templatePath] == null)
    {
      ...
      TemplateContainer templateContainer = YamlIO.LoadFile<TemplateContainer>(str + ".yaml");
  ...
  private static void GetAssetPaths(string folder, List<string> paths) => FileSystem.GetFiles(FileSystem.Normalize(System.IO.Path.Combine(TemplateCache.baseTemplatePath, folder)), "*.yaml", (ICollection<string>) paths);
  ...
  public static List<TemplateContainer> CollectBaseTemplateAssets(string folder = "bases")
  {
    ...
    foreach (string filename in (List<string>) pooledList)
      templateContainerList.Add(YamlIO.LoadFile<TemplateContainer>(filename));
    pooledList.Recycle();
    templateContainerList.Sort((Comparison<TemplateContainer>) ((x, y) => y.priority - x.priority == 0 ? x.name.CompareTo(y.name) : y.priority - x.priority));
