...
public class DropDown : KMonoBehaviour
{
  ...
  private void Build(List<IListableOption> contentKeys)
  {
    ...
    if (this.addEmptyRow)
    {
      ...
      string str = this.emptyRowLabel == null ? (string) STRINGS.UI.DROPDOWN.NONE : this.emptyRowLabel;
  ...
  public void Open()
  {
    ...
    Vector3 vector3 = this.dropdownAlignmentTarget.TransformPoint(this.dropdownAlignmentTarget.rect.x, this.dropdownAlignmentTarget.rect.y, 0.0f);
    Vector2 vector2 = new Vector2(Mathf.Min(0.0f, (float) Screen.width - (vector3.x + this.rowEntryPrefab.GetComponent<LayoutElement>().minWidth)), -Mathf.Min(0.0f, vector3.y - this.scrollRect.rectTransform().sizeDelta.y));
