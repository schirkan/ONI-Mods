...
public class EntombedItemManager : KMonoBehaviour, ISim33ms
{
  ...
  public void Sim33ms(float dt)
  {
    ...
    foreach (Pickupable pickupable in (HashSet<Pickupable>) pooledHashSet)
    {
      ...
      {
        SimMessages.AddRemoveSubstance(cell, (int) ElementLoader.FindElementByHash(elementId).idx, CellEventLogger.Instance.ElementConsumerSimUpdate, mass, temperature, diseaseIdx, diseaseCount);
      }
      else
      {
  ...
  private void AddMassToWorlIfPossible()
  {
    ...
    foreach (int item_idx in (List<int>) pooledList)
    {
      ...
      if ((double) obj.mass > 1.40129846432482E-45)
        SimMessages.AddRemoveSubstance(obj.cell, (int) ElementLoader.FindElementByHash((SimHashes) obj.elementId).idx, CellEventLogger.Instance.ElementConsumerSimUpdate, obj.mass, obj.temperature, obj.diseaseIdx, obj.diseaseCount);
    }
    pooledList.Recycle();
  }
