...
public class MinionStartingStats : ITelepadDeliverable
{
  ...
  public MinionStartingStats(bool is_starter_minion, string guaranteedAptitudeID = null)
  {
    if (is_starter_minion)
    {
      ...
      this.personality = Db.Get().Personalities[idx];
    }
    else
    {
      int idx = UnityEngine.Random.Range(0, 35);
      this.personality = Db.Get().Personalities[idx];
    }
  ...
  private int GenerateTraits(bool is_starter_minion, List<ChoreGroup> disabled_chore_groups)
  {
    ...
    int num1 = is_starter_minion ? 1 : 3;
    bool flag1 = false;
    while (!flag1)
    {
  ...
  {
    int a = Mathf.RoundToInt((float) ((double) Util.GaussianRandom() * ((double) DUPLICANTSTATS.MAX_STAT_POINTS - (double) DUPLICANTSTATS.MIN_STAT_POINTS) / 2.0) + (float) DUPLICANTSTATS.MIN_STAT_POINTS);
    ...
    for (int index = 0; index < list.Count; ++index)
    {
      ...
      int b = randomDistribution[Mathf.Min(index, randomDistribution.Length - 1)];
      int num = Mathf.Min(a, b);
      if (!this.StartingLevels.ContainsKey(key))
        this.StartingLevels[key] = 0;
      this.StartingLevels[key] += num;
      a -= num;
    }
