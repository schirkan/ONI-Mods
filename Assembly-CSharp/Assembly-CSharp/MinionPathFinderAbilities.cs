...
public class MinionPathFinderAbilities : PathFinderAbilities
{
  ...
  private static bool IsAccessPermitted(int proxyID, int cell, int from_cell) => !Grid.HasAccessDoor[cell] || Grid.HasPermission(cell, proxyID, from_cell);
    ...
    int underwater_cost)
  {
    ...
    bool flag1 = path.HasFlag(PathFinder.PotentialPath.Flags.PerformSuitChecks) && Grid.TryGetSuitMarkerFlags(from_cell, out flags, out pathFlags) && (uint) (flags & Grid.SuitMarker.Flags.Operational) > 0U;
