...
public class SeasonManager : KMonoBehaviour, ISim200ms
{
  ...
  {
    base.OnCleanUp();
    this.Unsubscribe<SeasonManager>(631075836, SeasonManager.OnNewDayDelegate);
  }
  ...
  private GameObject SpawnBombard(string prefab)
  {
    ...
    GameObject gameObject = Util.KInstantiate(Assets.GetPrefab((Tag) prefab), position, Quaternion.identity);
  ...
  {
    SeasonManager.Season season = this.seasons[this.SeasonLoop[this.currentSeasonIndex]];
    return season.bombardmentInfo != null && (uint) season.bombardmentInfo.Length > 0U;
  }
  ...
  public float TimeUntilNextBombardment()
  {
    ...
    return !this.bombardmentOn ? this.bombardmentPeriodRemaining : 0.0f;
  }
  ...
  public float GetBombardmentDuration()
  {
    if (!this.CurrentSeasonHasBombardment())
      return 0.0f;
    SeasonManager.Season season = this.seasons[this.SeasonLoop[this.currentSeasonIndex]];
    return !this.bombardmentOn ? season.secondsBombardmentOn.Get() : 0.0f;
  }
  ...
  [ContextMenu("Bombard")]
  public void Debug_Bombardment() => this.DoBombardment(this.seasons[this.SeasonLoop[this.currentSeasonIndex]].bombardmentInfo);
