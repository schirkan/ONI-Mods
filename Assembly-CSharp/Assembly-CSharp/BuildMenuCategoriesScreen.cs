    ...
    BuildMenuBuildingsScreen buildings_screen)
  {
    ...
    else if (typeof (IList<BuildMenu.DisplayInfo>).IsAssignableFrom(data.GetType()))
    {
      ...
      foreach (BuildMenu.DisplayInfo displayInfo in (IEnumerable<BuildMenu.DisplayInfo>) data)
      {
        ...
        BuildMenuCategoriesScreen.UserData userData = new BuildMenuCategoriesScreen.UserData();
        userData.category = displayInfo.category;
        userData.depth = depth;
        userData.requirementsState = PlanScreen.RequirementsState.Tech;
        int hotkey = (int) displayInfo.hotkey;
        string tooltip = (string) Strings.Get("STRINGS.UI.NEWBUILDCATEGORIES." + str + ".TOOLTIP");
        KIconToggleMenu.ToggleInfo toggleInfo = new KIconToggleMenu.ToggleInfo(text, icon, (object) userData, (Action) hotkey, tooltip);
