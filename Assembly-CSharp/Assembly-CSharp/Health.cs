...
public class Health : KMonoBehaviour, ISaveLoadable
{
  ...
  private void Recover() => this.GetComponent<KPrefabID>().RemoveTag(GameTags.HitPointsDepleted);
  ...
  public void RegisterHitReaction()
  {
    ...
    if (smi == null)
      return;
    SelfEmoteReactable reactable = new SelfEmoteReactable(this.gameObject, (HashedString) "Hit", Db.Get().ChoreTypes.Cough, (HashedString) "anim_hits_kanim", min_reactor_time: 1f, max_trigger_time: 1f);
  ...
  public void Incapacitate(Death source_of_death)
  {
    ...
    this.GetComponent<KPrefabID>().AddTag(GameTags.HitPointsDepleted);
  }
