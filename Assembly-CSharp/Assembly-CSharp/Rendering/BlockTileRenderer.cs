...
namespace Rendering
{
  public class BlockTileRenderer : MonoBehaviour
  {
    ...
    private static bool MatchesDef(GameObject go, BuildingDef def) => (UnityEngine.Object) go != (UnityEngine.Object) null && (UnityEngine.Object) go.GetComponent<Building>().Def == (UnityEngine.Object) def;
          ...
          for (int x = chunk_x * 16; x < chunk_x * 16 + 16; ++x)
          {
            ...
            if (this.occupiedCells.ContainsKey(num1))
            {
              ...
              for (int index = 0; index < this.atlasInfo.Length; ++index)
              {
                ...
                bool flag = (uint) (this.atlasInfo[index].forbiddenConnections & connectionBits) > 0U;
                if (num2 != 0 && !flag)
                {
        ...
        List<Color> colours)
      {
        for (int index1 = 0; index1 < this.decorInfo.decor.Length; ++index1)
        {
          ...
          if (decor.variants != null && decor.variants.Length != 0)
          {
            ...
            bool flag = (uint) (connection_bits & decor.forbiddenConnections) > 0U;
            if (num1 != 0 && !flag)
            {
