...
public class ElectricalUtilityNetwork : UtilityNetwork
{
    ...
    List<WireUtilityNetworkLink>[] bridgeGroups)
  {
    ...
    if (flag)
    {
      ...
      this.overloadedNotification = new Notification((string) MISC.NOTIFICATIONS.CIRCUIT_OVERLOADED.NAME, NotificationType.BadMinor, HashedString.Invalid, click_focus: this.targetOverloadedWire.transform);
      GameScheduler.Instance.Schedule("Power Tutorial", 2f, (System.Action<object>) (obj => Tutorial.Instance.TutorialMessage(Tutorial.TutorialMessages.TM_Power)), (object) null, (SchedulerGroup) null);
