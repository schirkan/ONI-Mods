...
public class CircuitManager
{
  ...
  public void Rebuild()
  {
    ...
    while (enumerator3.MoveNext())
    {
      ...
      int linked_cell1;
      current.GetCells(out linked_cell1, out int _);
  ...
  public void Sim200msLast(float dt)
  {
    ...
    for (int index1 = 0; index1 < this.circuitInfo.Count; ++index1)
    {
      ...
      this.UpdateBatteryConnectionStatus(circuitInfo.batteries, is_connected_to_something_useful1, index1);
      ...
      this.UpdateBatteryConnectionStatus(circuitInfo.inputTransformers, is_connected_to_something_useful2, index1);
      this.circuitInfo[index1] = circuitInfo;
      for (int index2 = 0; index2 < circuitInfo.generators.Count; ++index2)
      {
        Generator generator = circuitInfo.generators[index2];
        ReportManager.Instance.ReportValue(ReportManager.ReportType.EnergyWasted, -generator.JoulesAvailable, StringFormatter.Replace((string) BUILDINGS.PREFABS.GENERATOR.OVERPRODUCTION, "{Generator}", generator.gameObject.GetProperName()));
      }
    }
    for (int index = 0; index < this.circuitInfo.Count; ++index)
    {
...
label_7:
  ...
  public bool HasPowerSource(ushort circuitID)
  {
    ...
    if (generators.Count > 0 && (UnityEngine.Object) generators.Find(new Predicate<Generator>(this.FindActiveGenerator)) != (UnityEngine.Object) null)
      return true;
    return batteries.Count > 0 && (UnityEngine.Object) batteries.Find(new Predicate<Battery>(this.FindActiveBattery)) != (UnityEngine.Object) null;
  }
  ...
  private bool FindActiveGenerator(Generator g)
  {
    ...
    return component1.IsOperational && component2.IsPowered;
  }

  private bool FindActiveBattery(Battery b) => b.GetComponent<Operational>().IsOperational && (double) b.PercentFull > 0.0;
  ...
  public ReadOnlyCollection<Generator> GetGeneratorsOnCircuit(
    ushort circuitID)
  {
    return circuitID == ushort.MaxValue ? (ReadOnlyCollection<Generator>) null : this.circuitInfo[(int) circuitID].generators.AsReadOnly();
  }

  public ReadOnlyCollection<IEnergyConsumer> GetConsumersOnCircuit(
    ushort circuitID)
  {
    return circuitID == ushort.MaxValue ? (ReadOnlyCollection<IEnergyConsumer>) null : this.circuitInfo[(int) circuitID].consumers.AsReadOnly();
  }

  public ReadOnlyCollection<Battery> GetTransformersOnCircuit(
    ushort circuitID)
  {
    return circuitID == ushort.MaxValue ? (ReadOnlyCollection<Battery>) null : this.circuitInfo[(int) circuitID].inputTransformers.AsReadOnly();
  }
