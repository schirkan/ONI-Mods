...
namespace KMod
{
  public class Manager
  {
    ...
    public void Unsubscribe(Label label, object caller)
    {
      ...
      {
        Debug.LogFormat("\t...not found", (object[]) Array.Empty<object>());
      }
      else
      {
    ...
    public bool IsInDevMode() => this.mods.Exists((Predicate<Mod>) (mod => mod.enabled && mod.label.distribution_platform == Label.DistributionPlatform.Dev));
    ...
    public bool MatchFootprint(List<Label> footprint, Content relevant_content)
    {
      ...
      {
        Mod mod = this.mods[index];
        if ((uint) (mod.available_content & relevant_content) > 0U && mod.enabled)
        {
          this.events.Add(new Event()
          {
