...
public class VerticalWindTunnel : StateMachineComponent<VerticalWindTunnel.StatesInstance>, IGameObjectEffectDescriptor, ISim200ms
{
  ...
  protected override void OnPrefabInit()
  {
    ...
    this.bottomConsumer.OnElementConsumed += (System.Action<Sim.ConsumedMassInfo>) (info => this.OnElementConsumed(false, info));
    this.topConsumer = components[1];
    this.topConsumer.EnableConsumption(false);
    this.topConsumer.OnElementConsumed += (System.Action<Sim.ConsumedMassInfo>) (info => this.OnElementConsumed(true, info));
  ...
  protected override void OnSpawn()
  {
    ...
    for (int index = 0; index < this.workables.Length; ++index)
    {
      ...
      windTunnelWorkable2.OnWorkableEventCB = windTunnelWorkable2.OnWorkableEventCB + (System.Action<Workable.WorkableEvent>) (ev => this.OnWorkableEvent(player_index, ev));
  ...
  private Chore CreateChore(int i)
  {
    ...
    WorkChore<VerticalWindTunnelWorkable> workChore = new WorkChore<VerticalWindTunnelWorkable>(relax, (IStateMachineTarget) workable2, on_end: on_end, allow_in_red_alert: false, schedule_block: schedule_block, allow_prioritization: false, priority_class: PriorityScreen.PriorityClass.high);
  ...
  private void OnElementConsumed(bool isTop, Sim.ConsumedMassInfo info)
  {
    ...
    SimMessages.AddRemoveSubstance(Grid.OffsetCell(Grid.XYToCell((int) position.x, (int) position.y), offset), (int) info.removedElemIdx, CellEventLogger.Instance.ElementEmitted, info.mass, info.temperature, info.diseaseIdx, info.diseaseCount);
  }
