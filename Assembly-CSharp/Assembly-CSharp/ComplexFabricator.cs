...
public class ComplexFabricator : KMonoBehaviour, ISim200ms, ISim1000ms
{
  ...
  public bool HasWorker => !this.duplicantOperated || (UnityEngine.Object) this.workable.worker != (UnityEngine.Object) null;
  ...
  public bool IsRecipeQueued(ComplexRecipe recipe)
  {
    ...
    return (uint) recipeQueueCount > 0U;
  }
    ...
    DictionaryPool<Tag, float, ComplexFabricator>.PooledDictionary missingAmounts)
  {
    ...
    foreach (KeyValuePair<Tag, float> missingAmount in (Dictionary<Tag, float>) missingAmounts)
    {
      ...
      {
        FetchList2 fetchList2 = new FetchList2(this.inStorage, byHash);
        fetchList2.Add(missingAmount.Key, amount: missingAmount.Value);
        fetchList2.ShowStatusItem = false;
        fetchList2.Submit(new System.Action(this.OnFetchComplete), false);
        this.fetchListList.Add(fetchList2);
      }
    }
  }
...
label_7:
  ...
  protected virtual bool HasIngredients(ComplexRecipe recipe, Storage storage)
  {
    foreach (ComplexRecipe.RecipeElement ingredient in recipe.ingredients)
    {
      ...
      if ((double) ingredient.amount - (double) amountAvailable >= (double) PICKUPABLETUNING.MINIMUM_PICKABLE_AMOUNT)
        return false;
    }
    return true;
  }
    ...
    foreach (ComplexRecipe.RecipeElement result in recipe.results)
    {
      ...
      switch (this.resultState)
      {
        ...
        case ComplexFabricator.ResultState.Heated:
          ...
          component1.Temperature = this.resultState == ComplexFabricator.ResultState.PassTemperature ? num1 : this.heatedTemperature;
          go.SetActive(true);
      ...
      if (gameObjectList.Count > 0)
      {
        ...
        if ((UnityEngine.Object) component2 != (UnityEngine.Object) null)
        {
          ...
          if (symbol != null)
          {
            component2.TryRemoveSymbolOverride((HashedString) "output_tracker");
            component2.AddSymbolOverride((HashedString) "output_tracker", symbol);
          }
          else
