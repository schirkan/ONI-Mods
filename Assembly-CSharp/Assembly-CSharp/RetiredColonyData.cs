...
public class RetiredColonyData
{
    ...
    BuildingComplete[] buildingCompletes)
  {
    ...
    for (int index = 0; index < data12.Length; ++index)
    {
      ...
      data12[index] = new Tuple<float, float>((float) ReportManager.Instance.reports[index].day, num2 / (float) num1);
    }
    ...
    for (int index = 0; index < data13.Length; ++index)
    {
      ...
      {
        ++num1;
        num2 += entry.contextEntries[i].accNegative;
      }
      float num3 = num2 * -1f;
      data13[index] = new Tuple<float, float>((float) ReportManager.Instance.reports[index].day, num3 / (float) num1);
    }
