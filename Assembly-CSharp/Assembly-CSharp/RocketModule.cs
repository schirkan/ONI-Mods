...
public class RocketModule : KMonoBehaviour
{
  ...
  private void OnAttachmentNetworkChanged(AttachableBuilding ab) => this.FixSorting();
  ...
  private void DEBUG_OnDestroy(object data)
  {
    if (!((UnityEngine.Object) this.conditionManager != (UnityEngine.Object) null) || App.IsExiting || KMonoBehaviour.isLoadingScene)
      return;
    Spacecraft conditionManager = SpacecraftManager.instance.GetSpacecraftFromLaunchConditionManager(this.conditionManager);
    this.conditionManager.DEBUG_TraceModuleDestruction(this.name, conditionManager == null ? "null spacecraft" : conditionManager.state.ToString(), new StackTrace(true).ToString());
  }
  ...
  private void OnLaunch(object data)
  {
    ...
    foreach (ManualDeliveryKG component4 in this.GetComponents<ManualDeliveryKG>())
      component4.Pause(true, "Rocket in space");
  ...
  private void OnLand(object data)
  {
    ...
    foreach (ManualDeliveryKG component3 in this.GetComponents<ManualDeliveryKG>())
      component3.Pause(false, "landed");
  ...
  public virtual void OnSuspend(object data) => this.isSuspended = true;
  ...
  public string GetParentRocketName() => this.parentRocketName;
}
