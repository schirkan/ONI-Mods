...
public class MinionTodoSideScreen : SideScreenContent
{
    ...
    foreach (JobsTableScreen.PriorityInfo priorityInfo1 in MinionTodoSideScreen.priorityInfo)
    {
      ...
      if (priority == PriorityScreen.PriorityClass.basic)
      {
        for (int index = 5; index >= 0; --index)
        {
          ...
          tuple.third.name = "PriorityGroup_" + (string) priorityInfo1.name + "_" + (object) index;
  ...
  public override bool IsValidForTarget(GameObject target) => (UnityEngine.Object) target.GetComponent<MinionIdentity>() != (UnityEngine.Object) null && !target.HasTag(GameTags.Dead);
  ...
  private MinionTodoChoreEntry GetChoreEntry(RectTransform parent)
  {
    ...
    {
      minionTodoChoreEntry = Util.KInstantiateUI<MinionTodoChoreEntry>(this.taskEntryPrefab.gameObject, parent.gameObject);
