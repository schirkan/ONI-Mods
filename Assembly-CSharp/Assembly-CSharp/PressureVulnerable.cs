  ...
  public string WiltStateString
  {
    get
    {
      ...
      if (this.smi.IsInsideState((StateMachine.BaseState) this.smi.sm.warningLow) || this.smi.IsInsideState((StateMachine.BaseState) this.smi.sm.lethalLow))
        str += Db.Get().CreatureStatusItems.AtmosphericPressureTooLow.resolveStringCallback((string) CREATURES.STATUSITEMS.ATMOSPHERICPRESSURETOOLOW.NAME, (object) this);
        ...
        str += Db.Get().CreatureStatusItems.AtmosphericPressureTooHigh.resolveStringCallback((string) CREATURES.STATUSITEMS.ATMOSPHERICPRESSURETOOHIGH.NAME, (object) this);
      else if (this.smi.IsInsideState((StateMachine.BaseState) this.smi.sm.unsafeElement))
        str += Db.Get().CreatureStatusItems.WrongAtmosphere.resolveStringCallback((string) CREATURES.STATUSITEMS.WRONGATMOSPHERE.NAME, (object) this);
      return str;
    }
  }
  ...
  public List<Descriptor> GetDescriptors(GameObject go)
  {
    ...
    if (this.safe_atmospheres != null && this.safe_atmospheres.Count > 0)
    {
      ...
      foreach (Element safeAtmosphere in this.safe_atmospheres)
        str = str + "\n        • " + safeAtmosphere.name;
      descriptorList.Add(new Descriptor(string.Format((string) UI.GAMEOBJECTEFFECTS.REQUIRES_ATMOSPHERE, (object) str), string.Format((string) UI.GAMEOBJECTEFFECTS.TOOLTIPS.REQUIRES_ATMOSPHERE, (object) str), Descriptor.DescriptorType.Requirement));
    }
