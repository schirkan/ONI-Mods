...
public class AudioMixer
{
  ...
  public void StopPersistentSnapshots()
  {
    ...
    this.Stop((HashedString) AudioMixerSnapshots.Get().DuplicantCountAttenuatorMigrated);
    this.Stop((HashedString) AudioMixerSnapshots.Get().DuplicantCountMovingSnapshot);
    this.Stop((HashedString) AudioMixerSnapshots.Get().DuplicantCountSleepingSnapshot);
    this.Stop((HashedString) AudioMixerSnapshots.Get().SpaceVisibleSnapshot);
    this.Stop((HashedString) AudioMixerSnapshots.Get().FacilityVisibleSnapshot);
    this.Stop((HashedString) AudioMixerSnapshots.Get().PulseSnapshot);
  }
  ...
  private void SetVisibleDuplicants()
  {
    ...
    for (int idx = 0; idx < Components.LiveMinionIdentities.Count; ++idx)
    {
      if (CameraController.Instance.IsVisiblePos(Components.LiveMinionIdentities[idx].transform.GetPosition()))
      {
        ...
        {
          StaminaMonitor.Instance smi = Components.LiveMinionIdentities[idx].GetComponent<Worker>().GetSMI<StaminaMonitor.Instance>();
