...
namespace ProcGenGame
{
  ...
  public class TerrainCell
  {
    ...
    public void SetNode(ProcGen.Node newNode) => this.node = newNode;
    ...
    public virtual void LogInfo(string evt, string param, float value) => Debug.Log((object) (evt + ":" + param + "=" + (object) value));

    public static HashSet<int> GetClaimedCells() => TerrainCell.claimedCells;

    public static HashSet<int> GetHighPriorityClaimCells() => TerrainCell.highPriorityClaims;
    ...
    public void AddMob(KeyValuePair<int, Tag> mob)
    {
      ...
      if (!this.allCells.Contains(mob.Key))
      {
        ...
        {
          (object) "Couldnt find cell [",
          (object) mob.Key,
          (object) "] we dont own, to remove for mob [",
          null,
          null
        };
        tag = mob.Value;
        objArray[3] = (object) tag.Name;
        objArray[4] = (object) "]";
        Debug.Assert(false, (object) string.Concat(objArray));
      }
      else
      {
        object[] objArray = new object[5]
        ...
        if (!TerrainCell.highPriorityClaims.Contains(cell))
        {
          ...
          else
          {
            ...
            if (flag6 | flag5)
            {
              ...
              Vector2 b = vector2;
              erode = Mathf.Max(0.0f, Mathf.Min(1f, Vector2.Distance(a, b) / num2));
              if (flag6)
                erode = 1f - erode;
            }
            if (flag8 | flag7)
            {
              ...
              erode = Mathf.Max(0.0f, Mathf.Min(1f, b / num3));
              if (flag8)
                erode = 1f - erode;
            }
            if (flag9)
    ...
    {
      TerrainCell.SetValuesFunction SetValues = (TerrainCell.SetValuesFunction) ((index, elem, pd, dc) => SimMessages.ModifyCell(index, ElementLoader.GetElementIndex((elem as Element).id), pd.temperature, pd.mass, dc.diseaseIdx, dc.elementCount, SimMessages.ReplaceType.Replace));
      this.DoProcess(worldGen, world, SetValues, rnd);
    }
    ...
    public bool IsSafeToSpawnFeatureTemplate(Tag additionalTag) => !this.node.tags.Contains(additionalTag) && !this.node.tags.ContainsOne(TerrainCell.noFeatureSpawnTagSet);

    public bool IsSafeToSpawnFeatureTemplate() => !this.node.tags.ContainsOne(TerrainCell.noFeatureSpawnTagSet);
