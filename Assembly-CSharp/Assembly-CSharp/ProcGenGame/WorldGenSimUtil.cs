...
namespace ProcGenGame
{
  public static class WorldGenSimUtil
  {
      ...
      System.Action<Sim.Cell[], float[], Sim.DiseaseCell[]> onSettleComplete)
    {
      ...
      if (Sim.Load((IReader) new FastReader(bytes)) != 0)
      {
      ...
      {
        int callbackIdx = gameCell == Grid.CellCount - 1 ? 2147481337 : -1;
        SimMessages.ModifyCell(gameCell, (int) cells[gameCell].elementIdx, cells[gameCell].temperature, cells[gameCell].mass, dcs[gameCell].diseaseIdx, dcs[gameCell].elementCount, SimMessages.ReplaceType.Replace, callbackIdx: callbackIdx);
      }
      bool flag = false;
      while (!flag)
      {
      ...
      int num5 = WorldGenSimUtil.SaveSim(settings, data, error_cb) ? 1 : 0;
