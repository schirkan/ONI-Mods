...
namespace ProcGenGame
{
  ...
  public class WorldGen
  {
    ...
    public Vector2I SubWorldSize => this.data.subWorldSize;
    ...
    public void SaveSettings(string newpath = null) => SettingsCache.Save(newpath == null ? SettingsCache.GetPath() : newpath);
    ...
    public bool IsGenerateComplete() => this.generateThread != null && !this.generateThread.IsAlive;

    public bool IsRenderComplete() => this.renderThread != null && !this.renderThread.IsAlive;

    public void GenerateOffline()
    {
      int num1 = 1;
      for (int index = 0; index < num1 && !this.GenerateWorldData(); ++index)
      {
        DebugUtil.DevLogError("Failed worldgen");
        int num2 = this.successCallbackFn(UI.WORLDGEN.RETRYCOUNT.key, (float) index, WorldGenProgressStages.Stages.Failure) ? 1 : 0;
      }
    }

    private void PlaceTemplateSpawners(Vector2I position, TemplateContainer template) => this.data.gameSpawnData.AddTemplate(template, position);
            ...
            if (terrainCellsForTag.Count > 0)
            {
              ...
              if (templateContainer != null)
              {
                ...
                for (int index = 0; index < terrainCellsForTag.Count; ++index)
                {
                  ...
                  {
                    Vector2f size = templateContainer.info.size;
                    if ((double) size.Y > (double) terrainCell.poly.MaxY - (double) terrainCell.poly.MinY)
                    {
                      size = templateContainer.info.size;
                      float num1 = size.Y - (terrainCell.poly.MaxY - terrainCell.poly.MinY);
                      size = templateContainer.info.size;
                      float num2 = size.X - (terrainCell.poly.MaxX - terrainCell.poly.MinX);
    ...
    private void ReportWorldGenError(Exception e)
    {
      ...
      {
        errorDesc = string.Format((string) UI.FRONTEND.SUPPORTWARNINGS.WORLD_GEN_FAILURE, (object) settingsCoordinate),
        exception = e
      });
      KCrashReporter.ReportErrorDevNotification("WorldgenFailure", e.StackTrace, settingsCoordinate);
    }

    public void SetWorldSize(int width, int height) => this.data.world = new Chunk(0, 0, width, height);
    ...
    public bool GenerateLayout(WorldGen.OfflineCallbackFunction updateProgressFn)
    {
      ...
      try
      {
        ...
        if (!this.running)
          return false;
        Debug.Assert(this.data.world.size.x != 0 && (uint) this.data.world.size.y > 0U, (object) "Map size has not been set");
    ...
    public void UpdateVoronoiNodeTags(VoronoiTree.Node node) => (!node.tags.Contains(WorldGenTags.Overworld) ? this.WorldLayout.localGraph.FindNodeByID(node.site.id) : this.WorldLayout.overworldGraph.FindNodeByID(node.site.id))?.tags.Union(node.tags);
    ...
    public VoronoiTree.Tree GetOverworldForNode(Leaf leaf) => leaf == null ? (VoronoiTree.Tree) null : this.data.worldLayout.GetVoronoiTree().GetChildContainingLeaf(leaf);
      ...
      for (int index1 = 0; index1 < this.data.overworldCells.Count; ++index1)
      {
        ...
        for (int index2 = 0; index2 < vertices.Count; ++index2)
        {
          if (index2 < vertices.Count - 1)
            ...
            this.ConvertIntersectingCellsToType(new MathUtil.Pair<Vector2, Vector2>(vertices[index2], vertices[0]), vertices.Count > 4 ? "UNPASSABLE" : "EDGE");
        }
      }
      return true;
    }
    ...
    private List<Border> GetBorders(List<TerrainCell> cells)
    {
      ...
      for (int index = 0; index < cells.Count; ++index)
      {
        ...
        while (enumerator.MoveNext())
        {
          ...
          if (terrainCell != null)
            terrainCellSet.Contains(terrainCell);
          ++num;
        }
      }
      return borderList;
      ...
      WorldGen.OfflineCallbackFunction updateProgressFn)
    {
      ...
      try
      {
        ...
        {
          ProcGen.Map.Edge edge = edgesWithTag2[index];
          if (edge.site0 != edge.site1 && !edgesWithTag1.Contains(edge))
          {
            ...
            string key = string.IsNullOrEmpty(borderOverride2) || string.IsNullOrEmpty(borderOverride1) ? (!string.IsNullOrEmpty(borderOverride2) || !string.IsNullOrEmpty(borderOverride1) ? (!string.IsNullOrEmpty(borderOverride2) ? borderOverride2 : borderOverride1) : "hardToDig") : ((double) rnd.RandomValue() > 0.5 ? borderOverride2 : borderOverride1);
            borderList.Add(new Border(new Neighbors(a, b), edge.corner0.position, edge.corner1.position)
            {
              element = SettingsCache.borders[key],
      ...
      WorldGen.OfflineCallbackFunction updateProgressFn)
    {
      ...
      if (!boolSetting)
      {
        Math.Max(0, num4 - intSetting1 - 2 * intSetting2);
        num1 = -intSetting2;
        num2 = -intSetting2;
        num4 = world.size.y - 32;
      }
    ...
    {
      Debug.Assert(data != null && (uint) data.Length > 0U, (object) "MISSING DATA FOR NORMALIZE");
    ...
    private void GenerateUnChunkedNoise(WorldGen.OfflineCallbackFunction updateProgressFn)
    {
      ...
      int num2;
      NoiseMapBuilderCallback mapBuilderCallback = (NoiseMapBuilderCallback) (line => num2 = updateProgressFn(UI.WORLDGEN.GENERATENOISE.key, (float) (int) (0.0 + 25.0 * ((double) line / (double) this.data.world.size.y)), WorldGenProgressStages.Stages.GenerateNoise) ? 1 : 0);
      int num3;
    ...
    public void WriteOverWorldNoise(WorldGen.OfflineCallbackFunction updateProgressFn)
    {
      ...
      foreach (TerrainCell overworldCell in this.OverworldCells)
      {
        ...
        Rect bounds = overworldCell.poly.bounds;
        int width = (int) Mathf.Ceil(bounds.width + 1f);
        bounds = overworldCell.poly.bounds;
        int height = (int) Mathf.Ceil(bounds.height + 1f);
        bounds = overworldCell.poly.bounds;
        int num1 = (int) Mathf.Floor(bounds.xMin - 1f);
        bounds = overworldCell.poly.bounds;
        int num2 = (int) Mathf.Floor(bounds.yMin - 1f);
        Vector2 offset;
        Vector2 point = offset = new Vector2((float) num1, (float) num2);
        int num3;
        ...
        bounds = overworldCell.poly.bounds;
        double num4 = (double) (int) Mathf.Floor(bounds.xMin);
        local1.x = (float) num4;
        while (true)
        {
          double x1 = (double) point.x;
          bounds = overworldCell.poly.bounds;
          double num5 = (double) (int) Mathf.Ceil(bounds.xMax);
          if (x1 <= num5)
          {
            ref Vector2 local2 = ref point;
            bounds = overworldCell.poly.bounds;
            double num6 = (double) (int) Mathf.Floor(bounds.yMin);
            local2.y = (float) num6;
            while (true)
            {
              double y1 = (double) point.y;
              bounds = overworldCell.poly.bounds;
              double num7 = (double) (int) Mathf.Ceil(bounds.yMax);
              if (y1 <= num7)
              {
                if (overworldCell.poly.PointInPolygon(point))
                {
                  ...
                  intList.Add(cell);
                  int x2 = (int) point.x - num1;
                  int y2 = (int) point.y - num2;
                  this.BaseNoiseMap[cell] = noiseMap1.GetValue(x2, y2);
                  this.OverrideMap[cell] = noiseMap2.GetValue(x2, y2);
                  this.DensityMap[cell] = noiseMap3.GetValue(x2, y2);
        ...
        if (tree1.settings.normalise)
        {
          ...
          for (int index = 0; index < intList.Count; ++index)
            this.BaseNoiseMap[intList[index]] = (this.BaseNoiseMap[intList[index]] - b1) / num5;
        }
        if (tree2.settings.normalise)
        {
          float num5 = b4 - b3;
          for (int index = 0; index < intList.Count; ++index)
            this.OverrideMap[intList[index]] = (this.OverrideMap[intList[index]] - b3) / num5;
        }
        if (tree3.settings.normalise)
        {
          float num5 = b6 - b5;
          for (int index = 0; index < intList.Count; ++index)
            this.DensityMap[intList[index]] = (this.DensityMap[intList[index]] - b5) / num5;
        }
    ...
    public void SaveWorldGen()
    {
      try
      {
        ...
        worldGenSave.traitIDs = new List<string>((IEnumerable<string>) this.Settings.GetTraitIDs());
    ...
    public bool LoadWorldGen()
    {
      Klei.Data data;
      Dictionary<string, object> stats;
      this.wasLoaded = WorldGen.LoadWorldGen(out string _, out List<string> _, out data, out stats);
      if (this.wasLoaded)
      {
        this.data = data;
        this.stats = stats;
    ...
    public static bool LoadWorldGen(
      out string worldID,
      out List<string> traitIDs,
      out Klei.Data data,
      out Dictionary<string, object> stats)
    {
      data = (Klei.Data) null;
      stats = (Dictionary<string, object>) null;
      worldID = (string) null;
