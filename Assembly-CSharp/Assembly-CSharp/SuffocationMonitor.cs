...
public class SuffocationMonitor : GameStateMachine<SuffocationMonitor, SuffocationMonitor.Instance>
{
  ...
  public new class Instance : GameStateMachine<SuffocationMonitor, SuffocationMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
    ...
    public bool IsInBreathableArea() => this.master.GetComponent<KPrefabID>().HasTag(GameTags.RecoveringBreath) || this.master.GetComponent<Sensors>().GetSensor<BreathableAreaSensor>().IsBreathable();

    public bool HasSuffocated() => (double) this.breath.value <= 0.0;

    public bool IsSuffocating() => (double) this.breath.deltaAttribute.GetTotalValue() <= 0.0 && (double) this.breath.value <= 45.4545478820801;
