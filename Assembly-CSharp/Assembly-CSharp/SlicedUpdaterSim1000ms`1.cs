...
public abstract class SlicedUpdaterSim1000ms<T> : KMonoBehaviour, ISim200ms where T : KMonoBehaviour, ISlicedSim1000ms
{
  ...
  private int GetSliceIdx(T toBeUpdated) => toBeUpdated.GetComponent<KPrefabID>().InstanceID % this.m_slices.Count;
  ...
  public void RegisterUpdate1000ms(T toBeUpdated)
  {
    ...
    DebugUtil.DevAssert(slice.Count < this.maxUpdatesPer200ms, string.Format("The SlicedUpdaterSim1000ms for {0} wants to update no more than {1} instances per 200ms tick, but a slice has grown more than the SlicedUpdaterSim1000ms can support.", (object) typeof (T).Name, (object) this.maxUpdatesPer200ms));
  }
  ...
  private class Slice
  {
    ...
    public List<T> GetUpdateList()
    {
      ...
      objList.AddRange((IEnumerable<T>) this.m_updateList);
      objList.AddRange((IEnumerable<T>) this.m_recentlyAdded.Keys);
      return objList;
    }
