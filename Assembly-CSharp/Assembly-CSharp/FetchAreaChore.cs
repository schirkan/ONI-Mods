...
public class FetchAreaChore : Chore<FetchAreaChore.StatesInstance>
{
  ...
  private static bool IsPickupableStillValidForChore(Pickupable pickupable, FetchChore chore)
  {
    ...
    if (!component.HasAnyTags_AssumeLaundered(ref chore.tagBits))
    {
      ...
      Debug.Log((object) string.Format("Pickupable {0} is not valid for chore because it has none of these tags: {1}", (object) pickupable, (object) string.Join<Tag>(",", (IEnumerable<Tag>) tagsVerySlow)));
      return false;
    }
    if (!component.HasAllTags_AssumeLaundered(ref chore.requiredTagBits))
    {
      ISet<Tag> pickupableTags = (ISet<Tag>) component.Tags;
      List<Tag> list = chore.requiredTagBits.GetTagsVerySlow().Where<Tag>((Func<Tag, bool>) (x => !pickupableTags.Contains(x))).ToList<Tag>();
      Debug.Log((object) string.Format("Pickupable {0} is not valid for chore because it does not have the required tags: {1}", (object) pickupable, (object) string.Join<Tag>(",", (IEnumerable<Tag>) list)));
      return false;
    }
    if (!component.HasAnyTags_AssumeLaundered(ref chore.forbiddenTagBits))
      return true;
    ISet<Tag> pickupableTags1 = (ISet<Tag>) component.Tags;
    List<Tag> list1 = chore.forbiddenTagBits.GetTagsVerySlow().Where<Tag>((Func<Tag, bool>) (x => pickupableTags1.Contains(x))).ToList<Tag>();
    Debug.Log((object) string.Format("Pickupable {0} is not valid for chore because it has the forbidden tags: {1}", (object) pickupable, (object) string.Join<Tag>(",", (IEnumerable<Tag>) list1)));
    return false;
  }
  ...
  public class StatesInstance : GameStateMachine<FetchAreaChore.States, FetchAreaChore.StatesInstance, FetchAreaChore, object>.GameInstance
  {
    ...
    {
      TagBits tagBits = new TagBits(new Tag[2]
      {
        GameTags.Garbage,
    ...
    public void SetupDelivery()
    {
      ...
      else
      {
        ...
        {
          if ((UnityEngine.Object) x == (UnityEngine.Object) null || (double) x.TotalAmount <= 0.0)
            return true;
