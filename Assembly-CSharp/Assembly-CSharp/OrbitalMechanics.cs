...
public class OrbitalMechanics : KMonoBehaviour, IRenderEveryTick
{
  ...
  private void Rebuild()
  {
    ...
    {
      OrbitalMechanics.OrbitData data = this.orbitData[index];
      GameObject gameObject = Util.KInstantiate(Assets.GetPrefab((Tag) data.prefabTag), this.CalculatePos(ref data, time, out bool _));
    ...
    out bool behind)
  {
    ...
    float f = (float) ((this.applyOverrides ? (double) this.overridePercent / 100.0 : (double) time / (double) num1 - (double) (int) ((double) time / (double) num1)) * 2.0 * 3.14159274101257);
