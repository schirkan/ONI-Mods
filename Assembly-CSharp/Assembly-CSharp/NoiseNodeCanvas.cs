...
public class NoiseNodeCanvas : NodeCanvas
{
  ...
  public static NoiseNodeCanvas CreateInstance()
  {
    ...
    instance.ntf = YamlIO.LoadFile<NoiseTreeFiles>(NoiseTreeFiles.GetPath());
    return instance;
  }
  ...
  public override string DrawAdditionalSettings(string sceneCanvasName) => sceneCanvasName;
    ...
    NodeCanvas.CompleteLoadCallback onComplete)
  {
    ...
    if (GUILayout.Button(new GUIContent("Load Yaml", "Loads the Canvas from a Yaml Save File"), (GUILayoutOption[]) Array.Empty<GUILayoutOption>()))
    ...
    if (GUILayout.Button(new GUIContent("Save to Yaml", "Saves the Canvas to a Yaml file"), GUILayout.ExpandWidth(false)))
    {
      ...
      if (some_object != null)
      {
        ...
        YamlIO.Save<Tree>(some_object, treeFilePath);
      }
    }
    GUILayout.EndHorizontal();
    if (this.ntf == null)
      this.ntf = YamlIO.LoadFile<NoiseTreeFiles>(NoiseTreeFiles.GetPath());
    if (this.ntf != null && GUILayout.Button(new GUIContent("Load Tree", "Loads the Canvas from Trees list"), (GUILayoutOption[]) Array.Empty<GUILayoutOption>()))
    {
      ...
      genericMenu.Show(this.lastRectPos.position);
    }
  ...
  private ProcGen.Noise.Link GetLink(NodeEditorFramework.Node node)
  {
    ...
    if (type == typeof (PrimitiveNodeEditor))
    {
      ...
      Debug.Assert(primitiveNodeEditor.target.name != null && primitiveNodeEditor.target.name != "", (object) "Invalid target name");
    ...
    else if (type == typeof (FilterNodeEditor))
    {
      ...
      Debug.Assert(filterNodeEditor.target.name != null && filterNodeEditor.target.name != "", (object) "Invalid target name");
    ...
    else if (type == typeof (TransformerNodeEditor))
    {
      ...
      Debug.Assert(transformerNodeEditor.target.name != null && transformerNodeEditor.target.name != "", (object) "Invalid target name");
    ...
    else if (type == typeof (SelectorModuleNodeEditor))
    {
      ...
      Debug.Assert(moduleNodeEditor.target.name != null && moduleNodeEditor.target.name != "", (object) "Invalid target name");
    ...
    else if (type == typeof (FloatPointsNodeEditor))
    {
      ...
      Debug.Assert(pointsNodeEditor.target.name != null && pointsNodeEditor.target.name != "", (object) "Invalid target name");
  ...
  public Tree BuildTreeFromCanvas()
  {
    ...
    foreach (NodeEditorFramework.Node node in this.nodes)
    {
      ...
      {
        SelectorModuleNodeEditor moduleNodeEditor = node as SelectorModuleNodeEditor;
        if (moduleNodeEditor.target.name == null || moduleNodeEditor.target.name == "" || tree.selectors.ContainsKey(moduleNodeEditor.target.name))
      ...
      {
        ModifierModuleNodeEditor moduleNodeEditor = node as ModifierModuleNodeEditor;
        if (moduleNodeEditor.target.name == null || moduleNodeEditor.target.name == "" || tree.modifiers.ContainsKey(moduleNodeEditor.target.name))
      ...
      {
        CombinerModuleNodeEditor moduleNodeEditor = node as CombinerModuleNodeEditor;
        if (moduleNodeEditor.target.name == null || moduleNodeEditor.target.name == "" || tree.combiners.ContainsKey(moduleNodeEditor.target.name))
      ...
      {
        FloatPointsNodeEditor pointsNodeEditor = node as FloatPointsNodeEditor;
        if (pointsNodeEditor.target.name == null || pointsNodeEditor.target.name == "" || tree.floats.ContainsKey(pointsNodeEditor.target.name))
      ...
      {
        ControlPointsNodeEditor pointsNodeEditor = node as ControlPointsNodeEditor;
        if (pointsNodeEditor.target.name == null || pointsNodeEditor.target.name == "" || tree.controlpoints.ContainsKey(pointsNodeEditor.target.name))
  ...
  {
    NoiseNodeCanvas instance;
    NodeEditor.curNodeCanvas = (NodeCanvas) (instance = NoiseNodeCanvas.CreateInstance());
