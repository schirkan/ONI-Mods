...
namespace Steamworks
{
  public static class SteamGameServerApps
  {
    ...
    {
      InteropHelp.TestIfAvailableGameServer();
      IntPtr num = Marshal.AllocHGlobal(cubCommandLine);
      int launchCommandLine = NativeMethods.ISteamApps_GetLaunchCommandLine(CSteamGameServerAPIContext.GetSteamApps(), num, cubCommandLine);
      pszCommandLine = launchCommandLine != -1 ? InteropHelp.PtrToStringUTF8(num) : (string) null;
      Marshal.FreeHGlobal(num);
