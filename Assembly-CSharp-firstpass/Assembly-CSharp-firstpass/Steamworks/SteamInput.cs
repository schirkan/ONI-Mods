...
namespace Steamworks
{
  public static class SteamInput
  {
    ...
    {
      InteropHelp.TestIfAvailableClient();
      return handlesOut.Length == 16 ? NativeMethods.ISteamInput_GetConnectedControllers(CSteamAPIContext.GetSteamInput(), handlesOut) : throw new ArgumentException("handlesOut must be the same size as Constants.STEAM_INPUT_MAX_COUNT!");
    }

    public static InputActionSetHandle_t GetActionSetHandle(
      string pszActionSetName)
    {
      ...
      using (InteropHelp.UTF8StringHandle pszActionSetName1 = new InteropHelp.UTF8StringHandle(pszActionSetName))
        return (InputActionSetHandle_t) NativeMethods.ISteamInput_GetActionSetHandle(CSteamAPIContext.GetSteamInput(), pszActionSetName1);
    }
    ...
    public static InputActionSetHandle_t GetCurrentActionSet(
      InputHandle_t inputHandle)
    {
      InteropHelp.TestIfAvailableClient();
    ...
    {
      InteropHelp.TestIfAvailableClient();
      return handlesOut.Length == 16 ? NativeMethods.ISteamInput_GetActiveActionSetLayers(CSteamAPIContext.GetSteamInput(), inputHandle, handlesOut) : throw new ArgumentException("handlesOut must be the same size as Constants.STEAM_INPUT_MAX_ACTIVE_LAYERS!");
    }
      ...
      string pszActionName)
    {
      ...
      using (InteropHelp.UTF8StringHandle pszActionName1 = new InteropHelp.UTF8StringHandle(pszActionName))
        return (InputDigitalActionHandle_t) NativeMethods.ISteamInput_GetDigitalActionHandle(CSteamAPIContext.GetSteamInput(), pszActionName1);
    }
      ...
      string pszActionName)
    {
      InteropHelp.TestIfAvailableClient();
      using (InteropHelp.UTF8StringHandle pszActionName1 = new InteropHelp.UTF8StringHandle(pszActionName))
        return (InputAnalogActionHandle_t) NativeMethods.ISteamInput_GetAnalogActionHandle(CSteamAPIContext.GetSteamInput(), pszActionName1);
    }
    ...
    public static EInputActionOrigin TranslateActionOrigin(
