...
// Type: LibNoiseDotNet.Graphics.Tools.Noise.Primitive.Spheres
...
namespace LibNoiseDotNet.Graphics.Tools.Noise.Primitive
{
  public class Spheres : PrimitiveModule, IModule3D, IModule
  {
    ...
    public Spheres(float frequency) => this._frequency = frequency;
    ...
    public float GetValue(float x, float y, float z)
    {
      ...
      z *= this._frequency;
      double num1;
      double num2 = Math.Floor(num1 = Math.Sqrt((double) x * (double) x + (double) y * (double) y + (double) z * (double) z));
